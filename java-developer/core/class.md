...
Декларация класса включает в себя сокрытие внутренних полей и методов (инкапсуляция), описание сеттеров, геттеров и конструктора.
...
- Статические методы и атрибуты
  - `static` атрибут означает, что будет только 1 общий для всех экземпляров класса. Можно применить для подсчета количества экземпляров и т. п.
...
\[[Object]](java-developer/core/Object.md)
...
- Предназначен для установления контракта между клиентом класса
...
Анонимный класс — это вложенный класс, у которого нет имени, так как за основу берется существующий класс, который меняется разработчиком "на лету".
