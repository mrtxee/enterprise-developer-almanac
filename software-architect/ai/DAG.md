...
**DAG (Directed Acyclic Graph)** — это **основная концепция Apache [[airflow|Airflow]]**, лежащая в основе всего планирования и выполнения задач.
...
```mermaid
---
title: Пример DAG (Направленный Ациклический Граф)
---
flowchart LR
...
## Описание DAG в Airflow

На Python ([[airflow|Airflow]] использует код как конфигурацию):
...
```mermaid
---
title: Параллельное выполнение независимых задач в DAG
---
flowchart LR
    A[Начало] --> B[Задача 1]
    A --> C[Задача 2]
...
## Причина ацикличности

Если бы был цикл:

```mermaid
---
title: Циклический граф — не является DAG
---
flowchart LR
    A --> B
    B --> C
...
В веб-интерфейсе Airflow отображаются:
...
Без DAG'ов Airflow — просто [[airflow|планировщик]]. С DAG'ами — мощная система оркестрации данных.
