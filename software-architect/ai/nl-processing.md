...
**Обработка естественного языка** (Natural Language Processing, NLP) — это область [[ai-hierarchy|искусственного интеллекта]] и компьютерной лингвистики, занимающаяся взаимодействием между компьютерами и человеческим (естественным) языком.
...
- **[[word-embedding|Векторные представления слов]]** (Word2Vec, GloVe, FastText).
...
```mermaid
---
title: Этапы обработки текста в NLP
---
graph LR
...
| **[[hugging-face|Hugging Face]] Transformers** | Библиотека для BERT, GPT и других моделей |
...
Ниже — объяснение основных технологий в NLP и машинном обучении.
...
## Сравнение: эволюция технологий NLP
...
- Обеспечить последующий анализ: синтаксис, зависимости
- Служить основой для машинного перевода, чат-ботов
...
```mermaid
---
title: Дерево зависимостей для "She bought a red car"
---
graph TD
    bought(("bought")) --> She[She]
    bought --> a[a]
    bought --> car[car]
    car --> red[red]
```
...
- Показывает, кто кого купил
- Показывает, какой именно автомобиль
- Показывает, какие прилагательные относятся к существительному
...
```mermaid
---
title: POS, NER и Dependency Parsing
---
flowchart LR
...
Текст: `"Elon Musk founded Tesla in 2003"`
...
- формируется словарь всех слов (размер словаря = _V_);
- каждому слову присваивается индекс от 0 до _V_ − 1;
- слово кодируется вектором длины _V_, где на позиции его индекса стоит 1, остальные — 0.
...
- получается вектор частот длины _V_ (размер словаря).
...
4. Получается матрица _документы × слова_ с весами.
