...
> Retrieval-Augmented Generation — это **генерация, дополненная поиском**

**RAG** — это архитектурный паттерн в области [[nl-processing|NLP]], объединяющий *поиск информации* (retrieval) и *генерацию текста* (generation). Его цель — повысить точность и обоснованность ответов генеративных моделей за счёт привлечения внешних знаний.
...
4. **Генерация ответа**
   Языковая модель ([[large-language-model|LLM]]) анализирует объединённый контекст и формирует ответ, опираясь на фактические данные из найденных документов.
...
- **Retriever** — модуль поиска:
  ...
  Примеры: [[vector-store|FAISS]], Elasticsearch, Pinecone.
...
- **Generator** — языковая модель:
  ...
  Примеры: [[machine-learning-model|GPT]], Llama, Claude.
...
- Ограничить доступ: *«не показывать внутренние документы»*
- Улучшить ранжирование: *«новее = лучше»*
- Ответить пользователю: *«Информация из FAQ, стр. 12»*
...
```mermaid
---
title: Как всё работает вместе в RAG
---
graph TD
...
> *"RAG is only as good as your embeddings."*
...
Такой формат сохраняет логику последовательности этапов и делает информацию удобной для чтения в текстовом виде.
