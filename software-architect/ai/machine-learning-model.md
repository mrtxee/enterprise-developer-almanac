...
- Пример: вы раскладываете одежду для тёплой или холодной погоды. Такая модель классифицирует одежду как тёплую или холодную на основе данных, обозначенных на этикетке.
...
- Кластерные модели обучаются на основе немаркированных данных.
...
Прежде чем использовать модель, её необходимо обучить.
...
Модели глубокого обучения требуют больших объёмов обучающих данных и эффективны в таких задачах, как распознавание изображений, [[nl-processing|обработка естественного языка]] и составление более точных прогнозов.
...
**Большие языковые модели** (Large Language Models — LLMs) революционизируют [[ai-hierarchy|искусственный интеллект]], позволяя компьютерам понимать и генерировать текст, похожий на человеческий.

- Языковая модель извлекается из совокупности текстов, называемых **корпус** `corpus`.
...
**Нейросети** — это те же модели, которые мы упомянули ранее, но только с особенной структурой. Если представить графически, то это многослойная структура, состоящая из нейронов (функций), напоминающих биологические нейронные связи в человеческом мозге. Поэтому они и называются нейросетями.
