...
| Термин | Описание |
|--------|----------|
| **NLP (Natural Language Processing)** | Общий термин: всё, что связано с текстом: токенизация, POS Tagging, лемматизация |
| **NLU (Natural Language Understanding)** | Подмножество NLP. Фокусируется на **смысле**: намерение, контекст, диалоговое состояние |
...
| **Slot Filling** | Заполнить параметры действия | Пользователь: «Выключи свет» → команда: `turn_off_light`, room: `гостиная` |
| **Context Management** | Помнить контекст диалога | Пользователь: «А как насчёт завтра?» → что значит «завтра»? Только если помнить, о чём речь |
...
```mermaid
---
title: Конвейер обработки реплики пользователя
---
graph LR
...
> _NLP reads the text. NLU understands the user._

**NLU — основа всех умных систем**: чат-ботов, голосовых помощников, поиска по смыслу.
