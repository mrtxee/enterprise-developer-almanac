...
**AWS Glue Data Catalog** — это полностью управляемый сервис метаданных, который выступает в роли **централизованного репозитория** для хранения информации о данных в вашем дата-лейке (преимущественно в [[amazon-s3|Amazon S3]]).
...
```mermaid
---
title: Связь Glue Catalog и хранилища S3
---
erDiagram
    GLUE_CATALOG {
...
└── первая буква года/месяц в партициях
```

Например:

- `s3://my-datalake/raw/sales/year=2024/month=01/` → `data_part1.parquet`, `data_part2.parquet`;
- `s3://my-datalake/raw/sales/year=2024/month=02/` → `data_part3.parquet`;
- `s3://my-datalake/raw/sales/year=2025/month=01/` → `data_part4.parquet`.
...
```mermaid
---
title: Как Glue Crawler заполняет каталог
---
sequenceDiagram
...
### Пример: прямой доступ к метаданным через Boto3
...
| **Интеграция** | Athena, EMR, Redshift, Lake Formation | Spark, Hive, Presto | Spark, Trino, Dremio | Spark, Flink, Trino |
...
| **[[Hudi|Apache Hudi]]** | Через Spark/Hive | Upserts, incremental pulls |
| **[[delta-lake|Delta Lake]]** | Через Spark connector | ACID, time travel (требует Delta Runtime) |
...
Для работы с Iceberg в Glue Catalog требуется включить **Iceberg support** в настройках базы данных и использовать совместимые движки (Athena Engine v3, EMR 6.9+).

## Безопасность и управление доступом

### IAM-политики (базовый уровень)
...
```mermaid
---
title: Гранулярный доступ через Lake Formation
---
flowchart LR
...
```mermaid
---
title: Слоистая структура дата-лейка
---
flowchart TD
    Raw["📁 raw//table/"]
    Cleaned["📁 cleaned//table/"]
    Curated["📁 curated//subject/"]
...
| **Лимит 100k партиций на таблицу** | Перепроектируйте схему партиционирования (менее гранулярно) |
| **Стоимость при частых запросах** | Кэшируйте метаданные на стороне Spark (`spark.sql.hive.metastore.cacheTTL`) |
| **Нет версионирования схемы** | Храните DDL-скрипты в Git, используйте [[Nessie|Nessie]] для управления изменениями |
...
- вы в **мульти-клаудной среде** → **[[hive-metastore|Apache Hive Metastore]]** или **Iceberg REST Catalog**;
