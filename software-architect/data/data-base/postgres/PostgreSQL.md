...
Shared Memory (разделяемая память) — это специальный сегмент оперативной памяти, который используется PostgreSQL для **эффективного взаимодействия между процессами**. Это критически важный компонент архитектуры [[software-architect/data/data-base/rdbms/rdbms|СУБД]].
...
Visibility Map (карта видимости) — это специальная битовая карта, которая **оптимизирует работу механизма [[software-architect/data/data-base/rdbms/MVCC|MVCC]]** в PostgreSQL, отмечая страницы данных, где все строки видны всем активным транзакциям. Это критически важный компонент для производительности.
...
[[software-architect/data/data-base/rdbms/rdbms#✅ 6. **Оптимизация запросов и план выполнения**|Планировщик]] (Query Planner) — это ключевой компонент СУБД PostgreSQL, который преобразует SQL-запрос в оптимальный план выполнения, определяя наиболее эффективный способ доступа к данным.
...
- **Для текста (`text`, `varchar`)** — вычисляет расстояние между строками как [[trigram-similarity|метрику триграмм]]. Используется с расширением `pg_trgm` для **нечёткого поиска**.
...
1. **Поиск ближайших соседей ([[k-nn|KNN]])** — например, «найти 5 ближайших кафе к точке».
...
**[[sequence|Sequences]] в PostgreSQL** — это независимые объекты базы данных, предназначенные для автоматической генерации уникальных числовых значений.
