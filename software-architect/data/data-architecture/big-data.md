...
aliases:
  ...
  - Большие данные
---
## 📊 Big Data: руководство для новичков
...
### 🎯 Бизнес-ценность Big Data

| Цель | Как достигается | Реальный пример |
|------|-----------------|-----------------|
| **Персонализация** | Анализ поведения → рекомендации | Netflix: «Вам может понравиться…» |
| **Прогнозирование** | [[machine-learning|Машинное обучение]] на исторических данных | Банк: прогноз дефолта по кредиту |
...
### ⚙️ Архитектура Big Data

Типичная система состоит из **4 слоёв**:

```mermaid
---
title: Архитектура Big Data
---
flowchart TD
...
- **[[data-warehouse|Data Warehouse]]** — очищенные данные для аналитики
- **NoSQL БД** — для гибких схем (MongoDB, Cassandra)

#### Слой 3: Обработка

- **Пакетная (Batch)**: Spark, [[hive-ql|Hive]] — для исторических данных
- **Потоковая (Stream)**: Flink, [[kafka|Kafka]] Streams — для реального времени
- **Гибридная**: Lambda/Kappa архитектура

#### Слой 4: Анализ

- **BI-инструменты**: [[tableau|Tableau]], Power BI
- **[[machine-learning|Машинное обучение]]**: Python (scikit-learn), TensorFlow
...
|-----------|-------------|----------|
| **Хранение** | HDFS, [[S3|Amazon S3]], [[delta-lake|Delta Lake]] | Масштабируемое хранение |
| **Обработка** | Apache Spark, Flink | Анализ больших объёмов |
| **Потоки** | [[kafka|Apache Kafka]], Pulsar | Передача данных в реальном времени |
| **Оркестрация** | [[airflow|Airflow]], Prefect | Управление пайплайнами |
| **Каталог данных** | [[DataHub|DataHub]], Amundsen | Поиск и документация |
| **Облако** | AWS, GCP, Azure | Готовые сервисы (EMR, BigQuery) |

> 💡 **Совет новичку**:
> Начните с **облачных сервисов** (например, [[glue|AWS Glue]] + S3 + Athena) — не нужно настраивать кластеры вручную.
...
| **Финансы** | Обнаружение мошенничества | [[kafka|Kafka]] + Flink (stream processing) | Блокировка транзакции за 50 мс |
