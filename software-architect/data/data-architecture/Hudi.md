...
- **[[software-architect/data/data-base/rdbms/ACID|ACID]]-транзакций**

> **Ключевая идея**:
> *«Data Lake + транзакции = управляемое озеро данных»*
> Hudi добавляет «мозги» к объектному хранилищу ([[S3|S3]], ADLS, GCS), позволяя обрабатывать данные как в базе данных, но с масштабируемостью озера.

## Зачем Hudi: проблемы классических Data Lake
...
```mermaid
---
title: Copy-on-Write и Merge-on-Read
---
flowchart TB
...
### Индексы (для быстрых upsert)

| Тип индекса      | Скорость поиска | Память  | Использование                              |
| ---------------- | --------------- | ------- | ------------------------------------------ |
| **[[bloom-filter|Bloom Filter]]** | Быстро | Низкая | По умолчанию, для больших таблиц |
...
## Сравнение Hudi, Delta Lake и Iceberg

| Критерий                      | **Apache Hudi**                         | **[[delta-lake|Delta Lake]]**                             | **Apache [[Iceberg|Iceberg]]**               |
| ----------------------------- | --------------------------------------- | ---------------------------------------------- | ------------------------------------ |
...
| **Интеграция с потоками** | ✅ Отличная (встроенная поддержка [[kafka|Kafka]]) | ⚠️ Через Spark Streaming | ⚠️ Ограниченная |
...
|----------|--------------|-------------------|
| **CDC из базы данных** | Debezium → [[kafka|Kafka]] → Spark → Hudi | Обновления в реальном времени без полной перезаписи |
...
| **[[machine-learning|Машинное обучение]]** | Обучение на свежих данных из Hudi | Инкрементальная выборка только новых записей |
...
| **[[HiveQL|Hive]]** | ✅ Чтение | Через `hudi-hadoop-mr` |
...
> ➤ Для статичных данных без обновлений (просто используйте [[parquet|Parquet]])
