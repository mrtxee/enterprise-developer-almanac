...
**Apache Iceberg** — это **открытый формат таблиц для аналитических систем**, предназначенный для работы с **масштабными данными в объектных хранилищах** ([[S3|S3]], GCS, ADLS и др.). Он решает ключевые проблемы традиционных «озер данных» ([[data-lake|Data lakes]]), превращая их в **lakehouse** — гибрид озера и склада данных.
...
В традиционном data lake данные хранятся как файлы ([[parquet|Parquet]], CSV) в S3:
...
**Проблемы**
1. **Нет [[software-architect/data/data-base/rdbms/ACID|ACID]]-транзакций** — конкурентные записи ломают данные.
...
**Структура хранения**
- `s3://bucket/sales/data/` — файлы данных (Parquet/ORC)
- `s3://bucket/sales/metadata/`
...
| **Hive** | ⚠️ Ограниченная |
| **[[Dremio|Dremio]], Snowflake, BigQuery** | ✅ Через connectors |
...
| **Изоляция** | Можно использовать с [[Nessie|Nessie]] для ветвления (dev/prod) |

### Сравнение: Iceberg vs Delta Lake vs Hudi
...
### Типичная архитектура с Iceberg

```mermaid
---
title: Типичная архитектура с Iceberg
---
graph LR
...
**Заключение**
**Apache Iceberg — это «операционная система» для data lake.** Он превращает неструктурированное озеро файлов в **надёжную, согласованную и высокопроизводительную таблицу**, доступную через SQL.
