...
```mermaid
---
title: Архитектура Delta Lake
---
graph LR
    A[Источники данных<br>OLTP, API, IoT] --> B[Delta Lake<br>S3, HDFS]
    B --> C[Spark, Flink, Databricks]
    C --> D[BI Tools<br>например Power BI, Tableau]
    C --> E[ML/DS<br>например Python, PySpark]
...
Таблица `delta-table` в каталоге состоит из:

- `_delta_log/` — журнал изменений (файлы `000000000000.json`, `000000000001.json` и т. д.);
- `data_00000.parquet`, `data_00001.parquet` — сами данные.
...
**Delta Lake — это «один из способов реализовать Data Lakehouse».** Как **MySQL — это одна из баз данных для веб-приложения**.
...
> **Delta Lake — это технология** (формат). **Data Lakehouse — это архитектура** (система).
