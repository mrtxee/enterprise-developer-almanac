...
**Вывод:** реализация OLTP и OLAP процессинга над единым хранилищем данных является антипаттерном, т. к. процессы нацелены на взаимоисключающие цели. Это приводит к конфликту приоритетов: система не может одновременно оптимизировать и скорость мелких транзакций, и обработку массивных аналитических запросов.

**Архитектура: как они работают вместе**

```mermaid
---
title: Поток данных из OLTP в OLAP
---
graph TB
...
→ **[[extract-load-transform|ELT]]/Streaming** — для передачи данных из OLTP в OLAP
...
Данные для OLAP процессинга накапливаются в специальных хранилищах [[data-warehouse|Data warehouse]] (DW) путём [[extract-transform-load|ETL]] стриминга из OLTP хранилищ, например, MS SQL.
