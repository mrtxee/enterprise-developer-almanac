...
| **Load (Загрузка)** | Сохранение обработанных данных в целевую систему | [[data-warehouse|Data Warehouse]] (Snowflake), [[data-lake|Data Lake]], BI-система |
...
| **Соблюдение регламентов** | Анонимизирует [[personal-data|personal data]] перед загрузкой в аналитику |
...
```mermaid
---
title: Процесс ETL: от источников до целевых систем
---
flowchart LR
...
| Источники содержат **чувствительные данные** ([[personal-data|ПДн]]) | ➤ **ETL** (очистка до попадания в хранилище) |
...
> 📌 **Современный подход**:
> Применяется **гибридная модель** — базовые очистки в ETL, сложные трансформации в ELT (например, через **dbt** поверх данных в хранилище).
