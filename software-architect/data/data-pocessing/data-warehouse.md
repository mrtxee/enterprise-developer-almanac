...
- Собирает данные из разных источников ([[OLTP|OLTP]]-баз, файлов, API)
...
```mermaid
---
title: Поток данных в Data Warehouse
---
flowchart LR
...
| Компонент            | Роль                                     |
| -------------------- | ---------------------------------------- |
| **Источники данных** | [[OLTP|OLTP]]-базы, Excel, CSV, API           |
...
| **[[ELT|ELT]]**                        | Apache Airflow, Talend, Fivetran, dbt                   |
| **[[software-engineer/terms/abbreviations|BI]]** | Power BI, Tableau, Looker, Metabase                     |
| **Источники**                      | [[PostgreSQL|PostgreSQL]], MySQL, Oracle, CSV, [[kafka|Kafka]]                   |
...
```mermaid
---
title: Star Schema — факты и измерения
---
graph TD
...
| ❌ Не для операций | Не подходит для [[OLTP|CRUD-операций]] |
...
```mermaid
---
title: Структура схемы «Снежинка»
---
erDiagram
    FACT_SALES ||--o{ DIM_DATE : has
    FACT_SALES ||--o{ DIM_PRODUCT : has
...
## Современный подход: гибридная модель

На практике часто используют **гибридную модель**:

```mermaid
---
title: Гибридная модель — Star и Snowflake
---
erDiagram
    FACT_SALES ||--o{ DIM_DATE : has
    FACT_SALES ||--o{ DIM_PRODUCT : has
...
| **[[delta-lake|Delta Lake]] / [[Iceberg|Iceberg]]**               | Открытые форматы позволяют гибко менять структуру → меньше привязанности к одному паттерну     |
