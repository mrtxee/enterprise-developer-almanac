...
```mermaid
---
title: Пример запроса с трассировкой
config:
  theme: redux-color
  look: handDrawn
---
...
4. Интегрируйте с **мониторингом** ([[grafana|Grafana]], [[prometheus|Prometheus]]) для корреляции с метриками.
...
1. **Генерация Trace ID и Span ID**:
   - При поступлении запроса генерируется уникальный Trace ID, который связывает все спаны, относящиеся к этому запросу.
   - Каждый сервис, обрабатывающий запрос, создаёт Span с уникальным Span ID, который включает информацию о времени выполнения, метаданные (например, HTTP-метод, код ответа) и ссылки на родительский и дочерние спаны.

2. **Передача контекста между сервисами**:
   - Trace ID и Span ID передаются между сервисами через HTTP-заголовки, сообщения очередей или другие протоколы (например, W3C Trace-Context, B3 Zipkin Propagator).
...
3. **Сбор и экспорт данных**:
   - Для сбора трассировочных данных используются инструменты и SDK, такие как OpenTelemetry, Jaeger, Zipkin. Они предоставляют API и библиотеки для инструментирования кода.
   - OpenTelemetry, например, позволяет собирать данные о трассировке, метриках и логах, а затем экспортировать их в различные бэкенды через экспортеры (например, OTLPTraceExporter для отправки данных в коллектор).

4. **Обработка и хранение данных**:
   - Коллекторы (например, OpenTelemetry Collector, Jaeger Collector) получают данные от агентов или напрямую от приложений, обрабатывают их и отправляют в хранилище.
   - В качестве хранилищ часто используются NoSQL-СУБД ([[elastic-search|Elasticsearch]], [[cassandra|Cassandra]]), ClickHouse или другие системы, поддерживающие масштабируемое хранение и быстрый поиск.
   - Некоторые системы, например, Jaeger, могут использовать промежуточный слой (например, Apache Kafka) для буферизации данных перед их сохранением в хранилище.

5. **Визуализация и анализ**:
   - Данные из хранилища извлекаются через сервисы запросов (например, Jaeger Query) и визуализируются в пользовательских интерфейсах (например, Jaeger UI, Grafana) для анализа.

## Технологии для реализации

| Технология | Описание |
|------------|----------|
| **OpenTelemetry** | Универсальный стандарт для сбора, обработки и экспорта телеметрических данных. Поддерживает интеграцию с различными бэкендами и инструментами. |
| **Jaeger** | Открытый инструмент распределённой трассировки, разработанный Uber. Включает компоненты для сбора, хранения и визуализации данных. |
| **Zipkin** | Инструмент для сбора и анализа трассировочных данных, изначально разработанный Twitter. |
| **Elastic APM** | Часть Elastic Stack, поддерживает сбор трассировок, метрик и логов. Интегрируется с Elasticsearch для хранения данных. |
| **Grafana Tempo** | Открытый бэкенд для распределённой трассировки, интегрируется с Grafana для визуализации данных. |
| **AWS X-Ray**, **Google Cloud Trace**, **Azure Application Insights** | Облачные решения для распределённой трассировки от соответствующих провайдеров. |

## Дополнительные аспекты

- **Семплирование (Sampling)**: для снижения нагрузки на систему часто используется семплирование — сбор только части трассировок. Стратегии включают head-based (решение о сборе принимается в начале запроса), tail-based (после завершения запроса) и приоритетное семплирование.
- **Автоматизированная инструментиризация**: многие инструменты (например, OpenTelemetry) предоставляют библиотеки для автоматического сбора данных без необходимости вручную модифицировать код.
- **Интеграция с [[CI-CD|CI/CD]]**: рекомендуется внедрять инструментиризацию в процессе развёртывания новых версий приложений, чтобы обеспечить непрерывный мониторинг.
