...
**Kerberos** — это протокол аутентификации, который обеспечивает безопасную идентификацию пользователей и сервисов в сети. Его основная задача — подтвердить, что стороны, участвующие в обмене данными, действительно являются теми, за кого себя выдают. Используется, например, в [[active-directory|Windows Active Directory]] для управления доступом к ресурсам.
...
### Сценарии для Kerberos

#### Kerberos подходит, если
...
- SAML Docs — oasis-open.org
- Kerberos Docs — web.mit.edu/kerberos
- YouTube: _"SAML vs Kerberos"_ — TechWorld with Nana
