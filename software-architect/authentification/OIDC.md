...
OpenID Connect (OIDC) и [[oauth|OAuth]] 2.0 часто путают, но это **разные, хотя и тесно связанные, стандарты**. Ниже — чёткое сравнение по ключевым аспектам.

## Основная суть

- **[[oauth|OAuth 2.0]]** — протокол *авторизации* (делегирования доступа к ресурсам): дать приложению доступ к API/данным пользователя *без передачи пароля*.
  Пример: приложение просит у вас разрешение читать ваши фото в Google Фото.
- **OpenID Connect (OIDC)** — *надстройка над OAuth 2.0* для *аутентификации* (подтверждения личности): позволить пользователю войти в приложение, используя учётную запись провайдера (Google, GitHub и т. п.).
...
| **Что возвращает** | *Access token* (для вызова API) | *ID token* ([[JWT|JWT]] с данными пользователя) + *access token* |
...
- **Используйте [[oauth|OAuth 2.0]]**, если:
...
- **Используйте OIDC**, если:
  ...
  - Требуется стандартизированный способ аутентификации (например, для [[SSO|SSO]]).

## Пример потока (упрощённо)

1. Пользователь кликает «Войти через Google».
2. Приложение перенаправляет его на авторизационный эндпоинт Google (redirect к `/oauth2/v2/auth`) с параметрами:
...
- OIDC **не заменяет** [[oauth|OAuth 2.0]], а **дополняет** его, добавляя стандартизированную аутентификацию.
