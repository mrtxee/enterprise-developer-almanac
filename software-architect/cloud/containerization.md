...
[[docker]] работает так: приложение упаковывается со всеми зависимостями — библиотеками, интерпретаторами, файлами — в Docker-образ и отправляется в репозиторий (хранилище). Чтобы развернуть приложение, нужно скачать из репозитория образ и создать из него контейнер на рабочем сервере.
