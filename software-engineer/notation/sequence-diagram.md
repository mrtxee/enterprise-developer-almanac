---
aliases:
  - UML Sequence Diagram
  - Диаграмма взаимодействия
  - Диаграмма последовательности
---

## Диаграмма последовательности (Sequence Diagram)

**Sequence Diagram** — это диаграмма взаимодействия, показывающая **порядок обмена сообщениями** между объектами системы во времени. Она отвечает на вопрос «**в каком порядке** компоненты обмениваются вызовами», а не «какие классы существуют» (это решает [[UML|диаграмма классов]]).

## Когда она нужна

- Показать **последовательность вызовов** между модулями, сервисами, акторами.
- Описать **сценарий** выполнения операции: запрос → обработка → ответ.
- Зафиксировать **изменение состояния** участников.
- Показать **параллельные** или **условные** ветви (alt, opt, loop).

## Основные элементы

| Элемент | Описание |
|---------|----------|
| **Actor / Participant** | Участник: пользователь, сервис, модуль, БД |
| **Lifeline** | Пунктирная вертикальная линия участника, время |
| **Activation** | Прямоугольник активности на оси времени |
| **Message** | Стрелка вызова / сообщения |
| **Reply** | Ответное сообщение |
| **Self-call** | Рекурсивный вызов |
| **Fragment** | Блок alt / opt / loop для условной логики |

## Структура диаграммы

```mermaid
---
title: Общая структура диаграммы последовательности
---
sequenceDiagram
    participant A as Клиент
    participant S as Сервер
    participant D as База данных

    A->>S: HTTP запрос
    activate S
    S->>D: SELECT ...
    activate D
    D-->>S: результат
    deactivate D
    S-->>A: HTTP ответ
    deactivate S
```

- **Клиент** отправляет запрос → **Сервер** активируется.
- **Сервер** обращается к **БД** и получает результат.
- **Сервер** возвращает ответ клиенту.

## Как строить диаграмму

1. Определить **участников** (акторы, сервисы, системы).
2. Определить **сценарий**: что должно произойти.
3. Разложить на **события и сообщения** в хронологическом порядке.
4. Продумать условные ветви и циклы (fragments).
5. Проверить: каждый вызов имеет ответ или распределённую обработку.

## Элементы взаимодействия

```mermaid
---
title: Активация и возврат
---
sequenceDiagram
    participant M as Отправитель
    participant P as Приёмник

    M->>P: Вызов
    activate P
    P-->>M: Ответ
    deactivate P
```

- **Активация (activation)** — блок над линией жизни, показывающий занятость компонента.
- **Возврат (reply)** — пунктирная стрелка назад.
- **Self-call** — возврат к самому себе (повторная обработка внутри компонента).

## Условные фрагменты

```mermaid
---
title: Фрагмент alt (ветвление)
---
sequenceDiagram
    participant C as Клиент
    participant S as Сервис

    C->>S: Запрос данных
    alt Кэш существует
        S-->>C: Данные из кэша
    else Кэша нет
        S->>S: Запрос в БД
        S-->>C: Данные из БД
    end
```

```mermaid
---
title: Фрагмент loop (цикл)
---
sequenceDiagram
    participant C as Клиент
    participant S as Сервис

    loop Пока не получен ответ
        C->>S: Отправить повторный запрос
        S-->>C: Промежуточный ответ
    end
    C->>S: Финальный результат
```

## Синтаксис Mermaid

| Символ | Значение |
|--------|----------|
| `->>` | Синхронный вызов |
| `-->>` | Ответ / возврат |
| `-x` | Потеря сообщения (message loss) |
| `activate / deactivate` | Начало / конец активации |
| `alt / else` | Ветвление |
| `loop` | Цикл |
| `opt` | Опциональный блок |

## Пример: запрос к серверу

```mermaid
---
title: Запрос к серверу
---
sequenceDiagram
    participant U as Пользователь
    participant W as Веб-сервер
    participant A as API
    participant D as БД

    U->>W: GET /api/items
    activate W
    W->>A: внутренний запрос
    activate A
    A->>D: SELECT items
    activate D
    D-->>A: rows
    deactivate D
    A-->>W: JSON
    deactivate A
    W-->>U: 200 OK + body
    deactivate W
```

## Пример: запись в базу данных

```mermaid
---
title: Запись в БД
---
sequenceDiagram
    participant C as Клиент
    participant S as Сервис
    participant D as БД

    C->>S: POST /data
    activate S
    S->>S: Валидация
    S->>D: INSERT ...
    activate D
    D-->>S: OK (id)
    deactivate D
    S-->>C: 201 Created
    deactivate S
```

## Пример: генерация отчёта

```mermaid
---
title: Генерация отчёта
---
sequenceDiagram
    participant U as Пользователь
    participant A as Аналитика
    participant R as Отчёт

    U->>A: Запуск отчёта
    activate A
    A->>A: Подготовка данных
    A->>R: Агрегация
    activate R
    R-->>U: Ссылка на файл
    deactivate R
    deactivate A
```

## Особенности

- ✅ Наглядно показывает порядок вызовов и зависимости.
- ✅ Отражает тайминг и активацию участников.
- ✅ Поддерживает ветвления и циклы через фрагменты.
- ✅ Подходит для документации микросервисных сценариев.
- ❌ Не описывает внутреннюю структуру классов (это диаграмма классов).
- ❌ Не описывает инструкции с ветвлениями на уровне кода.
- ❌ Быстро перегружается при большом числе участников.

## Когда использовать

| Ситуация | Использовать? |
|----------|---------------|
| Документация API-сценария | ✅ Да |
| Разбор микросервисного взаимодействия | ✅ Да |
| Описание бизнес-процесса (пошагово) | ✅ Да |
| Объяснение жизненного цикла сущности | ✅ Да |
| Диаграмма классов (статическая структура) | ❌ Другая диаграмма ([[UML|uml]]) |
| ER-диаграмма БД | ❌ Другая диаграмма ([ER-диаграмма](ER-диаграмма)) |

## Связанные диаграммы

- [[UML|Диаграмма классов]] — структура объектов.
- [[UML|Диаграмма состояний]] — жизненный цикл объекта.
- [[event-driven-architecture|Диаграмма потоков]] — потоки данных и событий.
