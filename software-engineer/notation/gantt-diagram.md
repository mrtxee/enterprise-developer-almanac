---
aliases:
  - Gantt
  - Gantt diagram
  - диаграмма Ганта
  - Диаграмма Ганта
---

## Что такое диаграмма Ганта

**Диаграмма Ганта** — это популярный тип столбчатых диаграмм, который используется для иллюстрации плана, графика работ по проекту. Названа в честь американского инженера #👨 Генри Ганта, который разработал этот метод в 1910-х годах.

Пример простой диаграммы Ганта:

```mermaid
---
title: Пример диаграммы Ганта
---
gantt
    dateFormat  YYYY-MM-DD
    section Section
    A task           :a1, 2014-01-01, 30d
    Another task     :after a1  , 20d
    section Another
    Task in sec      :2014-01-12  , 12d
    another task      : 24d
```

## Основные компоненты диаграммы Ганта

**Ключевые элементы:**

- **Задачи/работы** — горизонтальные строки.
- **Временные интервалы** — горизонтальные полосы.
- **Вехи (Milestones)** — ключевые точки контроля.
- **Зависимости** — связи между задачами.
- **Прогресс выполнения** — заполнение полос.
- **Критический путь** — последовательность зависимых задач.

**Практические выгоды:**

- **Наглядность** — весь проект в одном представлении.
- **Точное планирование** — визуализация сроков.
- **Учёт зависимостей** — видны связи между задачами.
- **Отслеживание прогресса** — текущий статус выполнения.
- **Выявление проблем** — перекрытия, задержки, ресурсные конфликты.

## Типы диаграмм Ганта

### Базовая диаграмма Ганта

```mermaid
---
title: Базовая диаграмма Ганта
---
gantt
    dateFormat  YYYY-MM-DD
    section Проект А
    Исследование рынка   :a1, 2024-01-01, 30d
    Разработка ТЗ        :a2, after a1, 20d
    Создание прототипа   :a3, after a2, 25d
    section Проект Б
    Анализ требований    :b1, 2024-01-15, 20d
    Дизайн интерфейса    :b2, after b1, 30d
```

### Диаграмма с вехами

```mermaid
---
title: Диаграмма с вехами
---
gantt
    dateFormat  YYYY-MM-DD
    axisFormat  %m-%d

    section Разработка
    Планирование       :plan, 2024-01-01, 10d
    Проектирование     :design, after plan, 15d
    Разработка         :dev, after design, 30d
    Тестирование       :test, after dev, 20d

    section Вехи
    Утверждение ТЗ     :crit, milestone, after design, 0d
    Релиз версии 1.0   :crit, milestone, after test, 0d
```

### Диаграмма с прогрессом

```mermaid
---
title: Диаграмма с прогрессом
---
gantt
    dateFormat  YYYY-MM-DD

    section Проект
    Задача 1           :done,    task1, 2024-01-01, 20d
    Задача 2           :active,  task2, after task1, 25d
    Задача 3           :         task3, after task2, 15d
    Задача 4           :         task4, after task3, 10d
```

**Используйте диаграммы Ганта**, когда вам нужно:

- планировать сложные проекты с множеством зависимостей;
- визуализировать временные линии для [[stakeholder-map|стейкхолдеров]];
- отслеживать прогресс и выявлять потенциальные задержки;
- оптимизировать использование ресурсов команды.
