...
**Y-statement — форма записей ADR:**

1. Контекст (context) — на что решение влияет и в каких условиях принято. Это могут быть требования или условия работы конкретного компонента.
...
```mermaid
---
title: Этапы дизайн-мышления
---
graph TD
    A[1 Empathize<br/>Сопереживание]:::empathize
    B[2 Define<br/>Определение проблемы]:::define
    C[3 Ideate<br/>Генерация идей]:::ideate
    D[4 Prototype<br/>Создание прототипа]:::prototype
    E[5 Test<br/>Тестирование с пользователями]:::test
...
Задачу можно рассмотреть через дизайн-мышление и, возможно, найти вариант решения проблемы.

1. **Эмпатия.** Участники встречи пытаются определить источник проблемы клиента, встать на его место. Для начала нужно задать вопрос: «Зачем это нужно реализовать и какую проблему мы решаем?».
2. **Анализ и синтез.** После сбора информации о возникающих проблемах нужно проанализировать и систематизировать информацию, убрав всё лишнее. С одной стороны, проблему выявили, но стоит ещё раз убедиться, что эта проблема — верная.
3. **Генерация идей.** Нужно рассмотреть разные варианты решения проблемы, даже неожиданные.
4. **Прототипирование.** Нужно смоделировать решение проблемы любым способом (например, нарисовать на доске).
...
6. **Сторителлинг.** На последнем этапе нужно обобщить информацию об итоговом решении и сформировать итоговую историю, чтобы убедиться, что проблема будет решена.
