...
**Nginx** (произносится как *«engine-x»*) — это высокопроизводительный [[http-server|HTTP-сервер]], обратный прокси-сервер, балансировщик нагрузки и почтовый прокси-сервер.
...
2. **Обратный прокси ([[nginx-conf|Reverse Proxy]]):** Nginx принимает HTTP-запросы от клиентов и перенаправляет их на внутренние серверы (бэкенд). При этом пользователь не знает, на каком порту и каком языке написано ваше приложение.
...
Главное отличие Nginx от классических серверов (например, [[http-server|Apache]]) заключается в архитектуре:
...
[[nginx-conf|Конфигурация Nginx]] состоит из вложенных блоков. Основные директивы, которые нужно знать:
...
Nginx и [[http-server|Apache]] — два самых популярных веб-сервера в мире. Они решают одну и ту же задачу (отдача контента по HTTP/HTTPS), но используют совершенно разные подходы к архитектуре и обработке запросов.
...
Nginx полностью игнорирует `.htaccess`. Вся конфигурация задается централизованно в файлах `[[nginx-conf|nginx.conf]]`. Это требует прав root-пользователя для изменений, но работает намного быстрее.
