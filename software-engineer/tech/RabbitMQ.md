...
**Суть**
ActiveMQ, RabbitMQ и JMS — три разные сущности: JMS — это стандарт API, ActiveMQ и RabbitMQ — конкретные реализации message brokers. ActiveMQ реализует спецификацию JMS и поддерживает множество протоколов, RabbitMQ построен на протоколе AMQP.

## Основные концепции

Message brokers делятся на спецификацию (JMS) и реализации (ActiveMQ, RabbitMQ):

```mermaid
---
title: Message Brokers: JMS как спецификация, ActiveMQ и RabbitMQ как реализации
---
graph TD
...
Пример отправки сообщения через JMS API:
...
- ✅ **AMQP 0-9-1 протокол**
- ✅ **Мультиязычность** (Java, Python, .NET, Go и др.)
...
```mermaid
---
title: Архитектура JMS + ActiveMQ
---
graph LR
...
JMS — это API, а не протокол; он использует различные транспорты:
...
// HTTP
ConnectionFactory factory = new ActiveMQConnectionFactory("http://localhost:8080");
```

### Форматы сообщений JMS

JMS-сообщения делятся на шесть типов:
...
### Основной протокол RabbitMQ (AMQP 0-9-1)
...
### Plugin-протоколы RabbitMQ

Активация MQTT плагина:
...
RabbitMQ работает с бинарными payload; формат определяется приложением через Content-Type:
...
Тестовые показатели общей производительности:
...
### Производительность по протоколам ActiveMQ
...
### Производительность по протоколам RabbitMQ
...
- **JMS** — это стандарт API для Java messaging с поддержкой богатых форматов сообщений, но ограничен Java
- **ActiveMQ** — JMS-совместимый брокер с поддержкой multiple protocols, производительности и форматов; поддерживает наибольшее количество протоколов, идеален для гетерогенных сред
- **RabbitMQ** — высокопроизводительный AMQP-брокер для мультиязычных сред; фокусируется на AMQP с высочайшей производительностью, лучший для межъязыковой коммуникации

Ключевой выбор зависит от: требований к производительности, используемых языков программирования, разнородности клиентов, необходимых форматов сообщений и существующей инфраструктуры.
