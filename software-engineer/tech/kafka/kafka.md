...
Кафка стала популярной за счёт возможностей масштабирования и устойчивости к потерям данных. Кафка вошла в технологический ландшафт таких компаний, как LinkedIn, Netflix, Slack, Uber.

Масштабирование кафки достигается за счёт системы партиционирования, а устойчивость — за счёт распределённых [[replication|репликаций]].
...
flowchart RL
 ...
 subgraph ConsumerGroup["Consumer Group"]
    direction TB
        ConsumerGroup1_C1["Consumer 1<br>хост-клиент"]
        ConsumerGroup1_C2["Consumer 2<br>хост-клиент"]
  end
 ...
 subgraph Legend["Легенда"]
    ...
    Producer["Producer<br>хост-клиент"] -. produce .-> Broker1
...
`Kafka Cluster` — логическое объединение брокеров Kafka.

`Broker` — сервер Kafka, который хранит партиции, обрабатывает запросы клиентов и участвует в выборах лидера кластера.

`Topic` — канал, куда отправляются сообщения (например, «логи», «заказы»).

`Partition` — часть топика, упорядоченный журнал сообщений. Предназначен для параллельной обработки сообщений. В идеальной ситуации число партиций совпадает с числом потребителей Consumer Group.

`Offset` — номер сообщения внутри партиции. Позволяет консюмеру помнить, где остановился.

`Producer / Consumer` — клиент, который отправляет или потребляет сообщение топика.

`Consumer Group` — группа консюмеров, которые совместно читают один топик для балансировки нагрузки. С гарантией того, что два консюмера одной группы не прочитают одно сообщение.
...
> Для того чтобы клиент получил адреса и статус актуальных брокеров и партиций, при конфигурировании клиента задается список адресов брокеров (`bootstrap servers`). Если хотя бы один из них активен — клиент получит актуальную карту кластера.
...
Сообщения хранятся в бинарном формате в файловой системе. Имя папки: `<имя топика>-<номер партиции>`.
...
- `.TIMEINDEX` — служит для навигации в INDEX по времени. Слабая консистентность данных из-за того, что timestamp может устанавливаться вручную, либо сообщения могут приходить с задержкой.

Имя сегмента включает в себя номер сообщения, с которого он начинается. Стандартный размер сегмента — 1 Гб.

Удалять данные вручную нельзя. Но можно настроить удаление сегментов по TTL.

## Как работает Kafka

Для согласованности состояния кластера требуется координатор.

Ранее (до v.3.3) координатором выступал [[КRaft|Zookeeper]]. Сегодня такая конфигурация является устаревшей. Роль координатора (`Active controller`) передаётся между брокерами при помощи алгоритма консенсуса [[КRaft|KRaft]].
...
flowchart TB
 ...
 subgraph Kafka_Cluster["Kafka Cluster (логическое объединение брокеров)"]
    direction LR
        Broker1
        Broker2
        Broker3
 ...
 subgraph ConsumerGroup["Consumer Group"]
    direction TB
        ConsumerGroup1_C1["Consumer 1<br>хост-клиент"]
        ConsumerGroup1_C2["Consumer 2<br>хост-клиент"]
        ConsumerGroupCommment["Логическая группа consumer-ов с общим <code>__consumer_offsets</code>"]
        ConsumerGroup1_C3["Consumer 3<br>хост-клиент"]
  end
    ...
    Producer["Producer<br>хост-клиент"] -. "produce record<br>Topic-A Partition-0" .-> Broker1
...
Чтение и запись происходит только в `Leader Replica` партиции.
...
Leader реплика следит за Follower-ами репликации, отслеживает их отставание по LEO (Log End Offset) и ведёт локальный список In-Sync Replicas.

### Broker roles

Каждому брокеру назначается одна из 3 ролей (параметр `process.roles`):

- **broker** — обслуживает клиентов: принимает, хранит, отдаёт сообщения.
- **controller** — управляет кластером: выбирает лидеров партиций, обрабатывает ребалансы, хранит и синхронизирует метаданные (в режиме [[КRaft|KRaft]]).
- **broker, controller** — обе роли на одном узле. Типично для небольших кластеров.

> [!important] Passive broker
> При некоторых обстоятельствах может образоваться брокер, который содержит только Follower replica партиции. Чаще всего это временное, но штатное состояние системы. Таких брокеров также называют `cold replica broker`.
...
Уровень гарантии влияет на надёжность и длительность доставки сообщения. Поэтому значение следует задавать с учётом бизнес-требований:

- `acks=0`: без подтверждения доставки, риск потери данных, оптимально для телеметрии, логов, метрик. Fire-and-forget паттерн.
...
- `acks=-1`: подтверждения доставки от всех ISR реплик. Данные сохраняются даже при сбое лидера, снижение пропускной способности, оптимально для критических данных (например, платежи, заказы).
