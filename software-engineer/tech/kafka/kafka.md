...
Масштабирование кафки достигается за счёт системы партиционирования, а устойчивость — за счёт распределённых репликаций.
...
Ранее (до v.3.3) координатором выступал Zookeeper. Сегодня такая конфигурация является устаревшей. Роль координатора (`Active controller`) передаётся между брокерами при помощи алгоритма консенсуса KRaft.
