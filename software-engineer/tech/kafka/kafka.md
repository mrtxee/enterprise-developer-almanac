...
Ранее (до v.3.3) координатором выступал [[kraft|Zookeeper]]. Сегодня такая конфигурация является устаревшей. Роль координатора (`Active controller`) передаётся между брокерами при помощи алгоритма консенсуса [[kraft|KRaft]].
...
- **controller** — управляет кластером: выбирает лидеров партиций, обрабатывает ребалансы, хранит и синхронизирует метаданные (в режиме [[kraft|KRaft]]).
