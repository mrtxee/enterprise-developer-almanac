...
**Key Serde** в [[kafka|Apache Kafka]] — это сокращение от **"Serializer/Deserializer" для ключа сообщения**.
...
В [[kafka|Kafka]] каждое сообщение состоит из:
...
2. **Читаемость и типобезопасность**:
   Без Serde можно работать только с `byte[]`, что неудобно и небезопасно.
...
## Пример: важность согласованного формата

Если ключ отправляется как `String` (`"user123"`), а потребитель пытается прочитать его как `Long`, возникнет:
...
Без правильного Key Serde система может работать нестабильно или выдавать ошибки.
