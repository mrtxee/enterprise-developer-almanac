...
**Module Federation** — это функция Webpack 5, которая переносит [[microservice|микросервисную архитектуру]] во фронтенд. Она позволяет динамически подключать код из одного приложения в другое на этапе выполнения — без сборки всего в один бандл.
