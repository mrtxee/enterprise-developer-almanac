...
## Менеджер буфера обмена (Win+V)

Поиск пакетов менеджеров буфера обмена в репозитории Arch:
...
Переключение раскладки клавиатуры комбинацией `alt+shift`:

```bash
localectl set-x11-keymap us,ru pc104 , grp:alt_shift_toggle
```

## Установка рядом с GNOME

**Установка группы пакетов (если доступна в зеркале):**

```bash
sudo pacman -S cosmic
```

**Установка основных компонентов вручную:**
