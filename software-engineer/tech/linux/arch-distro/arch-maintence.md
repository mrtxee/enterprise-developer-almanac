...
- **`topgrade`** — обновляет не только пакеты Arch, но и сторонние менеджеры (Flatpak, Snap, Pip, Rust и др.). Умеет запускаться по таймеру и выводить уведомления об успешном завершении. Устанавливается через AUR (`yay -S topgrade`).
