...
- [[bios-uefi|BIOS и UEFI]] **инициализируют оборудование** и выбирают [[software-engineer/tech/linux/arch-distro/bootloader|загрузчик]].
...
1. Включение питания → **firmware** (UEFI) читает настройки из [[uefi-loader-setup|NVRAM]].
...
| **[[partitions|ESP]]** | Скрытый раздел `EFI System Partitio`, файлы `*.efi` |
| **shim** | Прослойка для Secure Boot (подпись GRUB) |
| **GRUB** | Мультизагрузчик: умеет грузить ядро Linux, Windows, другие ОС |
| **[[linux-kernel|Kernel]]** | Ядро Linux: управляет процессами, памятью, драйверами |
