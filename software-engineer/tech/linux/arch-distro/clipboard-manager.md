...
**Менеджер буфера обмена** в среде Wayland (например, в GNOME на Arch Linux) реализуется на основе **cliphist** — истории буфера обмена, работающей с `wl-clipboard` (`wl-paste`/`wl-copy`) и QEMU, а также поискового интерфейса **wofi** и эмулятора ввода **wtype**.
...
## Конфигурация `~/.config/cliphist/config`

```bash
max-items 94
```

## Автозапуск `~/.config/autostart/cliphist.service`
...
## Скрипт поиска `~/.local/clipmenu/clipmenu`
...
**Горячая клавиша:** `super+v` → `~/.local/clipmenu/clipmenu`.
