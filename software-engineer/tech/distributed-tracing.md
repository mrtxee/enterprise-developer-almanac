...
- Была ли проблема в `OrderService`?
...
Сразу видно: **email-сервис тормозит — 500мс! И упал.**
...
|---|---|
| **[[open-telemetry\|OpenTelemetry]] (OTel)** | **Стандарт де-факто** — объединяет OpenTracing и OpenCensus. Поддерживает 10+ языков. Рекомендуется CNCF. |
...
3. **Order Service** → создаёт заказ → создаёт `Span: create_order` → отправляет `OrderCreated` в [[kafka|Kafka]]
...
Вы знаете: **проблема в Email Service — он не может подключиться к SMTP**. Не нужно лазить в логи 5 сервисов — **всё на одном экране**.
...
| Проблема | Решение |
|---|---|
| **Накладные расходы** | Трассировка увеличивает нагрузку на сеть и [[cpu-gpu|CPU]] → включайте только для production-трассировок |
...
|---|---|---|
| **Protobuf / Avro** | Формат **данных** внутри сообщений (events, [[RPC|gRPC]]) | Эти данные передаются между сервисами — и должны быть корректно сериализованы |
...
- **Kafka + [[avro|Avro]] + Schema Registry** — для событий (надёжность, совместимость)
- **OpenTelemetry + Jaeger** — для трассировки всего пути (от frontend до kafka consumer)

```mermaid
---
title: Связь Avro / Protobuf и Distributed Tracing
---
graph LR
