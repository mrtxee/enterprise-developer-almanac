...
| `test` | Только для тестов | ❌ Нет | [[junit|JUnit]], Mockito, AssertJ |
...
- **В [[Docker|Docker]]/[[kubernetes|Kubernetes]]:** `provided` помогает не дублировать библиотеки, которые уже есть в базовом образе (например, сервлет-контейнер в Tomcat), чтобы итоговый WAR был компактнее.
