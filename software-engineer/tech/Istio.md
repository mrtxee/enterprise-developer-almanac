...
- **Envoy Proxy** — прокси-сервер на основе паттерна [[sidecar]], размещается рядом с каждым микросервисом. Отвечает за маршрутизацию трафика, балансировку нагрузки, мониторинг и безопасность. Функции:
...
| [[istio]] | Конкретная реализация [[service-mesh]] для [[kubernetes]] |

> Service Mesh — это концепция. Istio — один из самых популярных инструментов для её реализации. Как HTTP — это протокол, а Nginx — его реализация. Или: Pub/Sub — это паттерн, а Kafka — его реализация.
...
| Компонент | Роль |
| --- | --- |
| Envoy | [[sidecar]]-прокси (data plane). Отвечает за HTTP/gRPC-проксирование, mTLS, логирование, retry, timeout, circuit breaker |
...
Service Mesh возможен и без Istio. Все прокси при этом настраиваются ручными конфигами (JSON/YAML), управление ведётся через Ansible или Custom Controller, метрики собираются через Prometheus + Grafana, mTLS — через Cert-Manager + CA.
