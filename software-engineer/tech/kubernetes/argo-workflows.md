...
**Суть**
«Argo» в контексте Kubernetes — это не один инструмент, а семейство [[CNCF|Cloud Native Computing Foundation]]-проектов для декларативного управления рабочими процессами (workflows), CI/CD, GitOps и запуска задач в кластере. Это стек, который помогает превращать код в работающий сервис с контролем и повторяемостью.
...
Совершенный GitOps-контроллер для декларативного деплоя приложений в Kubernetes. Состояние кластера должно совпадать с тем, что описано в Git.
...
Типичный полный цикл CI/CD в Kubernetes:
