...
- **Role (роль)** — набор разрешений, который определяет, какие действия можно выполнять над ресурсами в определённом namespace.
- **ClusterRole** — аналогичен Role, но применяется ко всему кластеру. Используется для управления ресурсами, которые не привязаны к определённому namespace, например `nodes` или `persistentvolumes`.
- **RoleBinding** — связывает пользователя, группу или сервисный аккаунт с ролью (Role) в конкретном namespace и предоставляет разрешения, которые определены в роли. Связывает созданную роль с конкретным пользователем или сервисным аккаунтом.
...
> «With great power comes great responsibility.» — но в K8s: «Without RBAC — you have no control.»
