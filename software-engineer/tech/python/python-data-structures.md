...
Ниже — реализации `Stack`, `Queue`, `Tree`, `Linked List`.
...
Связанный список — это такой список, каждый элемент которого состоит из 2 ячеек: данные `data` и ссылка на следующий элемент связного списка `link`. Первый элемент такого списка называется голова `head`. Хвост списка имеет ссылку на `None`. Эта структура подходит для реализации undo/redo функции программы.
...
**Граф** — это множество связанных узлов, в котором каждый узел называется **вершиной** `vertex`, а связь между узлами — **ребром** `edge`.
