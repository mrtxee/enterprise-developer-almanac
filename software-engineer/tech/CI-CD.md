...
1. **Работа с системой контроля версий (Code).** Разработчики фиксируют изменения кода в системе контроля версий — например, в [[git|Git]]. Это позволяет отслеживать изменения, управлять различными ветками разработки и координировать работу нескольких разработчиков.
...
[[pipeline-as-code|pipeline-as-code]] — это подход, при котором конвейеры CI/CD описывают с помощью кода. Управляют ими тоже через код.
...
- ✅ Меньше инфраструктуры (один [[helm|Helm]]-чарт, один Ingress).
...
- Используются [[service-mesh|Service Mesh]], Event Bus, [[api-gateway|API Gateway]].
...
| ✅ Интеграция с [[istio|Istio]], [[kafka|Kafka]], Prometheus | Полный контроль трафика, трассировки, метрик |
...
| Зависимости | Внутренние — управляемы | Внешние — требуют контрактов ([[open-api|OpenAPI]], [[avro|Avro]]) |
...
| Legacy-система                                        | Монолит + постепенная миграция в микросервисы ([[strangler-fig|Strangler Pattern]])   |
