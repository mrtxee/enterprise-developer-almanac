...
```mermaid
---
title: Как работает XMLHttpRequest
---
sequenceDiagram
    participant JS as JavaScript
    participant XHR as XMLHttpRequest Object
    participant Net as Browser Network Stack

    JS->>XHR: new XMLHttpRequest()
    XHR->>Net: open(), send()
    Note over Net: DNS lookup, TCP/TLS, HTTP request/response
    Net-->>XHR: response
...
**Синхронный запрос (не рекомендуется)**
...
xhr.onload = function() {
    // Успех, если сервер вернул правильные заголовки
    console.log(xhr.response);
};

xhr.onerror = function() {
    // Ошибка, если CORS не настроен
...
| Актуален ли сейчас? | ⚠️ Устаревает, но поддерживается |
