...
1. Настроить переменную `cluster` для переключения между кластерами.
2. Задать алерты: «CPU > 95 % на 10 мин → Slack».
3. Поделиться дашбордом с командой.
