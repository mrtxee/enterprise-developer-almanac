...
- адаптацию качества под скорость сети (если интернет падает, видео становится хуже, но звонок не рвётся).
...
```mermaid
---
title: Установление WebRTC-соединения
---
sequenceDiagram
...
|---|---|---|
| **Signaling Server** | Обмен SDP (описание сессии) и ICE кандидатами до начала звонка. | ❌ Нет (только метаданные) |
| **STUN Server** | Помогает узнать свой публичный IP-адрес. Бесплатные и открытые (например, от Google). | ❌ Нет (только запрос-ответ) |
...
3. **Масштабирование P2P:**
   ...
   - 1→N (вебинар на 1000 человек): P2P не подойдёт (компьютер стримера не выдержит 1000 исходящих потоков). Нужна архитектура **SFU** (Selective Forwarding Unit) — сервер, который принимает один поток и рассылает его всем.
...
| **Совместная работа** | Доски (Miro), совместное редактирование кода, удалённая поддержка (screen sharing) |
| **Финтех** | Верификация личности через видео-звонок ([[KYC|KYC]]) |
