...
| Примеры IP-адресов | комментарий                                        |
| ------------------ | -------------------------------------------------- |
...
Пакеты могут идти разными путями, прийти в разном порядке — но это уже задача **[[tcp-udp|TCP]]** (или игнорируется при **[[tcp-udp|UDP]]**).
...
| **Сборка** | Не занимается! Это задача [[tcp-udp|tcp-udp]]/UDP (IP просто доставляет пакеты) |
...
| **Длина адреса** | 32 бита (~4.3 млрд адресов) | 128 бит (~340 undecillion адресов — почти бесконечно!) |
...
| В браузере | Зайдите на `whatismyipaddress.com` — покажет ваш **публичный IP** |
