...
**MIME** (Multipurpose Internet Mail Extensions) — стандарт, который расширяет возможности интернета по передаче различных типов данных. Первоначально создан для электронной почты, но сейчас используется повсеместно.
...
```mermaid
---
title: Иерархия основных MIME-типов
---
mindmap
  root((MIME Types))
    text
...
# Quoted-Printable (для текста со спецсимволами)
...
#### Email с HTML и альтернативным текстом
...
#### REST API с различными форматами
...
- Передавать любые типы данных по интернет-протоколам.
- Определять, как обрабатывать полученные данные.
- Обеспечивать совместимость между разными системами.
- Повышать безопасность за счёт явного указания типов.

Без MIME интернет был бы ограничен только текстовыми сообщениями. Благодаря MIME можно отправлять фото, видео, документы, программы и любые другие данные через email и веб.
