...
Он используется для **HTTPS**, [[mime|SMTP]], FTPS и других безопасных соединений.

Протокол TLS/mTLS на диаграмме последовательности:
...
```mermaid
---
title: Как TLS работает для сайтов
---
sequenceDiagram
...
| Замена паролей/ключей на сертификаты | → **mTLS** |

> **mTLS — стандарт для внутреннего взаимодействия в secure-сетях.**

## Пример: без mTLS vs с mTLS
...
**Через Service Mesh (лучший способ):**
...
> «In a microservices world, if you're not using mTLS, you're just pretending to be secure.» — Google Cloud Security Team
...
Схема клиентской и серверной частей:
...
## Структура TLS-сертификата (X.509 v3)

Краткая структура TLS-сертификата (стандарт X.509 v3) состоит из трёх основных блоков:

1. **Данные владельца (Тело сертификата):**
   - **Версия, серийный номер, алгоритм подписи.**
   - **Издатель (Issuer):** кто выдал (например, Let's Encrypt).
   - **Субъект (Subject):** кому выдан (домен сайта).
   - **Период действия:** дата начала и окончания.
   - **Публичный ключ:** сам ключ сайта и его алгоритм (RSA/ECC).

2. **Расширения (X.509 v3):**
   - **SAN (Subject Alternative Name):** список всех доменов, которым подходит сертификат (самое важное для HTTPS).
   - **Key Usage:** для чего можно использовать ключ (например, только шифрование).
   - **CRL/OCSP:** адреса для проверки отзыва сертификата.

3. **Цифровая подпись:**
   - Хеш от первых двух блоков, зашифрованный **закрытым ключом Удостоверяющего центра (CA)**. Браузер проверяет её публичным ключом CA, чтобы убедиться, что сертификат не подделан.
