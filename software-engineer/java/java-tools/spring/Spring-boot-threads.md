...
- [[kafka|Kafka]]/RabbitMQ: пулы потоков для слушателей.
- WebClient/RestTemplate: пулы для HTTP-клиентов.
- [[spring-actuator-probes|Actuator]]: служебные потоки для метрик и health checks.
...
3. Используйте реактивные стеки ([[web-flux|WebFlux]] + Netty) для высокой параллельности при малом числе потоков.
