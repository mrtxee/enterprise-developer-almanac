...
### RestTemplate — устаревший стандарт

`RestTemplate` был основным HTTP-клиентом Spring на протяжении десятилетия. Сейчас он **переведён в режим поддержки** и не получит новых фич.
...
### RestClient — современная замена RestTemplate
...
### WebClient — для реактивных и асинхронных сценариев
...
Для всех новых синхронных проектов применяется `RestClient`. `RestTemplate` сохраняется только для поддержки легаси. `WebClient` используется, когда нужна реактивность или максимальная производительность при большом количестве одновременных запросов.
