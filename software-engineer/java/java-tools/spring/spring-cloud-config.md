...
> 💡 **Идея:** конфигурация — это код. Храните её в Git, версионируйте, проводите ревью, тестируйте.
...
```mermaid
---
title: Архитектура Spring Cloud Config
---
flowchart TD
    Git["Git / SVN / Vault / JDBC"] --> ConfigServer["Config Server (Spring Boot)"]
    ConfigServer -->|HTTP/HTTPS| ServiceA["Service A<br>Config Client, @RefreshScope"]
    ConfigServer -->|HTTP/HTTPS| ServiceB["Service B<br>Config Client, @RefreshScope"]
    ConfigServer -->|HTTP/HTTPS| ServiceC["Service C<br>Config Client, @RefreshScope"]
    Bus["Spring Cloud Bus<br>(RabbitMQ / Kafka)"] -->|broadcast /refresh| ServiceA
    Bus -->|broadcast /refresh| ServiceB
    Bus -->|broadcast /refresh| ServiceC
```

Config Server читает конфиги из Git / SVN / Vault / JDBC и экспонирует REST API: `/{app}/{profile}/{label}`. Клиенты обращаются к нему по HTTP/HTTPS.
...
1. Конфиг меняется в Git, изменения пушатся.
2. Отправляется запрос: `POST http://service:8080/actuator/refresh`.
...
### Spring Cloud Bus (broadcast на все сервисы)
...
Отправляется один запрос `POST /actuator/busrefresh` — все сервисы обновляются.
...
Spring Cloud Config — стандарт де-факто для управления конфигурациями в Spring-микросервисах. При правильной настройке он даёт:
