...
**Суть**
Для автоматических повторов при исключениях в Spring используется библиотека **`spring-retry`** — пошаговая инструкция с production-готовым примером.

### Добавление зависимостей

Нужны **два** артефакта: сам `spring-retry` и `spring-boot-starter-aop` (без него `@EnableRetry` не создаст прокси).
...
## Потоковая модель Spring Retry

### Блокирующий повтор в том же потоке

Если при вызове метода происходит сбой и только на 9-й попытке выполнение завершается успешно, повтор происходит в том же потоке, который вызвал метод:

1. Поток ловит исключение.
2. Поток ждёт задержку (`Thread.sleep()`).
3. Поток повторно вызывает метод.
...
```mermaid
---
title: Схема выполнения повторов в одном потоке
---
flowchart TB
    A[Вызывающий поток: http-nio-8080-exec-3] --> B[@Retryable interceptor]
    B --> C[1-я попытка: fetchData]
    C -->|IOException| D[BackOffPolicy.backOff]
    D --> E[Thread.sleep 1000 — тот же поток]
    E --> F[2-я попытка: fetchData]
    F -->|IOException| G[Thread.sleep 2000]
    G --> H[... повторы 3-8 ...]
    H --> I[9-я попытка: fetchData — успех]
    I --> J[Возврат результата вызывающему коду]
```

### Что происходит с Thread.sleep()
...
> ⚠️ **Ключевой момент:** `Thread.sleep()` блокирует **именно тот поток**, который выполняет метод.
...
Если основной поток не должен блокироваться, есть два пути.

**Вариант 1: `@Retryable` + `@Async`**
...
**Вариант 2: `RetryTemplate` + `TaskExecutor`**
...
> Если метод — **фоновая задача** (`@Scheduled`, очередь) — синхронные повторы допустимы.
