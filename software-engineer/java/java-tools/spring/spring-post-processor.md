...
**BeanFactoryPostProcessor** срабатывает раньше всех — ещё до создания любых бинов. Это самый ранний этап в жизненном цикле Spring-приложения.

### Полная временная шкала жизненного цикла Spring

```mermaid
---
title: Порядок инициализации Spring context
---
flowchart TB
    A[ApplicationContext создаётся] --> B[BeanFactoryPostProcessor.postProcessBeanFactory]
    B --> C[Модификация BeanDefinition]
    C --> D[BeanPostProcessor регистрируются]
    D --> E[Создание singleton-бинов]
    E --> F[Constructor]
    F --> G[postProcessBeforeInitialization]
    G --> H[@PostConstruct, InitializingBean, init-method]
    H --> I[postProcessAfterInitialization]
    I --> J[ContextRefreshedEvent публикуется]
    J --> K[ApplicationRunner / CommandLineRunner выполняются]
    K --> L[Приложение работает]
    L --> M[ContextClosedEvent при shutdown]
```

### Визуализация порядка инициализации
...
**Профилирование bean definition'ов**
...
**PropertySourcesPlaceholderConfigurer (встроенный в Spring)**
...
`BeanFactoryPostProcessor` работает на уровне **метаданных** бинов (BeanDefinition), а не на уровне **экземпляров**. Это позволяет менять конфигурацию приложения ещё до того, как будут созданы какие-либо бины.

---

## Контекстные события и runner-интерфейсы
...
Используйте `ApplicationRunner` для большинства задач инициализации в Spring Boot-приложениях. Он предоставляет более удобный доступ к аргументам командной строки.
