...
- оптимизаций [[jvm|JVM]] (dead code elimination, JIT-компиляция);
...
2. **Сравнение реализаций:** например, `ArrayList` vs `LinkedList`, разные методы [[hash-function|хеширования]].
...
- **[[junit|JUnit]] + ручные замеры:** подходит для простых случаев, но менее точный.
