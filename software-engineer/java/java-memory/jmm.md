...
> [[bool-algebra#Бинарное отношение]]

##### Отношение HB выполняется для

- Синхронизация и мониторы `volatile`
- Запись и чтение
...
- JVM делит память на стеки потоков (thread stacks), кучу (heap) и meta space.
...
- `visibility` [[multithreading#visibility — видимость разделяемых объектов]]
- `race condition` [[multithreading#race condition — состояние гонки]]
