...
**Memory Barrier** (также известный как **Memory Fence**) — это механизм, который обеспечивает порядок выполнения операций с памятью в многопоточных программах. В Java они неявно используются в механизмах синхронизации.

---

## Проблема переупорядочивания операций
...
## Создание Memory Barriers в Java
...
- **Вход в [[object|монитор]]** → LoadLoad + LoadStore барьеры
...
### Проблема видимости без барьеров
...
## Happens-Before и Memory Barriers
...
> **Важно:** В большинстве случаев достаточно использовать высокоуровневые механизмы (`volatile`, `synchronized`, `AtomicXXX`), и [[jvm|JVM]] автоматически вставит нужные барьеры памяти.
