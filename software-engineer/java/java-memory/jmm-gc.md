...
3. **JNI ссылки** — объекты, на которые ссылаются нативные методы через Java Native Interface.
4. Мониторы и **объекты синхронизации**:
   - `synchronized` блоки;
   - объекты в состоянии `wait()` или `notify()`.
5. Объекты, загруженные через системный загрузчик классов **Bootstrap ClassLoader**:
