...
Экземпляры класса `Class` представляют классы и интерфейсы в работающем Java-приложении. Enum-класс и record-класс — виды классов; annotation-интерфейс — вид интерфейса. Каждый массив также принадлежит классу, представленному объектом `Class`, общим для всех массивов с одинаковым типом элемента и количеством измерений. Примитивные типы Java (boolean, byte, char, short, int, long, float, double) и ключевое слово `void` также представлены объектами `Class`.

Объект `Class` создаётся автоматически виртуальной машиной Java при загрузке класса из байт class-файла.
