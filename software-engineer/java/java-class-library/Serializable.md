...
Для успешной сериализации через `ObjectMapper.writeValueAsString()` класс или record должен быть инстанцируемым и иметь доступные для чтения свойства (геттеры, публичные поля или конструктор с `@JsonProperty`). `Serializable` не требуется.
