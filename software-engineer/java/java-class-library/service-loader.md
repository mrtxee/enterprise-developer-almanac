...
- **Логирование**: библиотека SLF4J использует SPI, чтобы найти и подключить конкретную реализацию (Logback, Log4j2) без жёсткой привязки к ней в коде.
- **Плагины**: создание расширяемых приложений, куда можно просто добавить JAR-файл с новой реализацией интерфейса, и приложение подхватит её автоматически.
...
3. **Java 9+ (модульная система JPMS)** — вместо файлов в `META-INF/services` модуль-провайдер объявляет `provides com.example.GreetingService with com.example.impl.EnglishGreeting;` в `module-info.java`, а модуль-потребитель — `uses com.example.GreetingService;`. Загрузка происходит через `ServiceLoader.load(GreetingService.class, ModuleLayer.boot())`.

**Краткий итог**

Используйте `ServiceLoader`, когда нужно реализовать **слабую связность (loose coupling)** и дать возможность подключать разные реализации одного и того же интерфейса без изменения кода основного приложения (принцип инверсии зависимостей).
