...
Терминальный метод `collect()` интерфейса `Stream` служит для того, чтобы перейти от потоков к привычным [[java-collection-framework|коллекциям]] — `List<T>`, `Set<T>`, `Map<T, R>` и другим. В метод `collect()` нужно передать специальный объект — collector.
