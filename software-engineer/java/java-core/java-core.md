...
Девиз Java: между исходным кодом и исполнением стоит компиляция в байт-код, который передается в виртуальную машину Java ([[jvm|JVM]]), взаимодействующую с ОС. JVM можно адаптировать под любую ОС, что гарантирует работу одного кода на разных платформах.
...
- Все методы всегда внутри класса — [[oop|ООП]], строгая типизация.
...
Для создания потока класс должен наследоваться от `Thread` [[java-thread|Thread]].
...
Запечатанными могут быть интерфейсы и записи. Фича поддерживается с [[jdk-jls-jni|JDK]] 17.
