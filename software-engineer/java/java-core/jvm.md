...
| GC (сборщик мусора)               | G1 ZGC Shenandoah CMS Parallel                                  | Зависит от режима (обычно G1/ZGC)                                | Eclipse OOM (Optimized for low pause)          | [[jmm-gc\|C4]] (Continuously Concurrent Compacting Collector) — почти без пауз |
...
- Ограничения при использовании [[reflection-api|рефлексии]]
...
| ------------------------------------------------------ | ------------------------------------ |
| Обычный enterprise-сервис (Spring Boot [[javax-vs-jakarta|Jakarta EE]])     | HotSpot (OpenJDK) — безопасный выбор |
