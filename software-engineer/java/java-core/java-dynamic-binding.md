...
- [[jvm|JVM]] определяет **реальный тип объекта** и вызывает соответствующую версию метода `sound()`

---

## Детальный разбор механизма

### Определение вызываемого метода JVM
...
- `Parent obj = new Child();` — объявление ссылки типа `Parent`
- `obj.show();` — компилятор проверяет, что метод `show()` определён в `Parent`

**Выполнение**

1. JVM определяет реальный класс объекта: `obj` → `[Child]`.
2. Ищет `show()` в таблице методов класса `Child`.
3. Вызывает метод из `Child`.
...
### Фабричный метод с динамическим связыванием
...
| Класс  | Наследует | Методы                                       |
| ------ | --------- | -------------------------------------------- |
| Animal | —         | `sound() → Animal`                           |
| Dog    | Animal    | `sound() → Dog` (переопределён), `toString()` |
| Cat    | Animal    | `sound() → Cat` (переопределён), `toString()` |
...
### Динамическое связывание не работает с полями
...
### Работает только для обычных методов
...
```mermaid
---
title: Выбор типа связывания
---
flowchart TD
    A["Вызов метода"] --> B{"Метод static, private или final?"}
    B -- "Да" --> C["Статическое связывание (compile-time)"]
    B -- "Нет" --> D["Динамическое связывание (runtime)"]
    C --> E["По типу ссылки"]
    D --> F["По реальному типу объекта"]
```
...
Это один из ключевых механизмов [[oop|ООП]] в Java, делающий код гибким и расширяемым без изменения существующего кода (принцип открытости/закрытости).
