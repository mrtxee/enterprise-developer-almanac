...
Непроверяемые исключения вызваны проблемами, которые не могут быть обработаны и обращены. Например, «закончилась память» — `OutOfMemoryError`. Мы не можем увеличить объем памяти в [[jvm|JVM]] — это необратимое событие.
