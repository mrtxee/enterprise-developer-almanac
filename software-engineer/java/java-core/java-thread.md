...
Взаимосвязь уровней потоков:

```mermaid
---
title: Уровни абстракции потоков
---
flowchart TD
    A["Spring Boot Thread (Java)"] --> B["JVM Thread (абстракция)"]
    B --> C["OS Native Thread (pthread/WinAPI)"]
    C --> D["CPU Hardware Thread (логическое ядро)"]
```

1. Spring Boot создаёт Java‑поток (например, для обработки HTTP‑запроса).
2. JVM запрашивает у ОС создание нативного потока через системный вызов.
3. ОС выделяет ресурсы и ставит поток в очередь планировщика.
4. Планировщик ОС назначает поток на свободный логический процессор (ядро/HT).
5. Процессор выполняет инструкции потока, используя свои регистры и кэш.

## Java Thread

> Поток в Spring Boot

Это объект класса `java.lang.Thread` — единица выполнения на уровне [[jvm|JVM]]. В Spring Boot потоки используются для:
...
- внутренних нужд фреймворка (мониторинг [[spring-actuator-probes|Actuator]] и т. д.).
...
flowchart LR
    ...
    Running -. sleep()<br>wait()<br>suspend()<br>join() .-> Blocked(["Waiting"])
    Blocked -. time completed<br>notify()<br>resume() .-> Runnable
...
```java
yield() // "уступить". Просим JVM выделить меньше времени на выполнение потока
...
Но это не очень хорошо, так как нарушает [[clean-code|SRP]] ([[SOLID|SOLID]]) ибо у нас появляется сущность которая управляет и потоком и выполняет дополнительные действия.
