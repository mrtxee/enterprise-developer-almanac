...
> ⚠️ **Важно:** буква `T` — обязательный разделитель между датой и временем. В некоторых контекстах (например, [[internet-date-time|RFC 3339]]) допускается пробел, но канонический ISO 8601 требует `T`.
