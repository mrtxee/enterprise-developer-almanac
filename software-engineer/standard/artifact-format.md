...
#### Общая карта экосистем

```mermaid
---
title: Общая карта экосистем пакетов
---
graph TB
...
```mermaid
---
title: Системные пакетные менеджеры
---
graph LR
...
```mermaid
---
title: Категории языковых пакетных менеджеров
---
graph TB
...
```mermaid
---
title: Размер пакетов по категориям
---
graph LR
...
```mermaid
---
title: Выбор пакетного менеджера по языку
---
flowchart TD
...
```mermaid
---
title: Матрица зрелости экосистем пакетов
---
quadrantChart
...
- **Системные**: apt, yum
- **Языковые**: maven, npm, pypi, cargo, go, nuget, composer, rubygems, cocoapods, conan, conda
...
**Совет:** Не сравнивайте форматы напрямую — они решают разные задачи. apt/yum для ОС, maven/npm/pypi для языков, docker для контейнеров, helm для Kubernetes. Выбор определяется языком и платформой.

---

## Структура пакетов: формат архива и манифест

Каждый пакетный формат — это архив определённого типа, содержащий файл манифеста с метаданными пакета.
...
#### Формат DOCKER (Container Images)
...
##### Wheel (.whl) — бинарный дистрибутив
...
##### Source Distribution (.tar.gz) — исходный дистрибутив
...
- **Расположение:** N/A.
- **Максимум:** **0** (нет стандарта). Если пользователь положил JSON/YAML файлы — это его собственная конвенция, не стандарт формата.
