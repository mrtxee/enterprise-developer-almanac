...
RFC 9110 описывает **правила кэширования в HTTP** — как клиенты и промежуточные серверы (прокси, [[CDN|CDN]]) должны сохранять и переиспользовать ранее полученные ответы.
...
```mermaid
---
title: Алгоритм работы HTTP-кэша
---
flowchart LR
...
#### Статические ресурсы (кэшируем надолго)
...
#### API-ответы (кэшируем недолго с валидацией)
...
#### Секретные данные (никакого кэша)
