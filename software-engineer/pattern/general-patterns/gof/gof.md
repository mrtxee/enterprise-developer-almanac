...
[[pattern|Паттерны]] GoF решают часто встречающиеся проблемы проектирования и делятся на три категории:

- Порождающие (Creational) — как создавать объекты.
- Структурные (Structural) — как компоновать объекты и классы.
- Поведенческие (Behavioral) — как организовать взаимодействие и распределение ответственности.
...
**Цель**
Создавать семейства связанных объектов без привязки к конкретным классам.
...
```mermaid
---
title: Abstract Factory — диаграмма классов
---
classDiagram
...
**Цель**
Пошаговое создание сложного объекта с возможностью варьировать представление.
...
```mermaid
---
title: Builder — диаграмма классов
---
classDiagram
class Director {
setBuilder(builder)
...
**Цель**
Делегировать создание объекта подклассам.
...
```mermaid
---
title: Factory Method — диаграмма классов
---
classDiagram
class Document {
<<abstract>>
...
**Цель**
Клонировать существующий объект вместо создания нового.
...
```mermaid
---
title: Prototype — диаграмма классов
---
classDiagram
class Prototype {
<<interface>>
...
**Цель**
Гарантировать единственный экземпляр класса и глобальную точку доступа к нему.
...
```mermaid
---
title: Singleton — диаграмма классов
---
classDiagram
class Logger {
- instance : Logger
...
**Цель**
Сделать несовместимые интерфейсы совместимыми.
...
```mermaid
---
title: Adapter — диаграмма классов
---
classDiagram
class Target {
<<interface>>
...
**Цель**
Отделить абстракцию от реализации, чтобы их можно было изменять независимо.
...
```mermaid
---
title: Bridge — диаграмма классов
---
classDiagram
class Shape {
renderer : Renderer
...
**Цель**
Обрабатывать отдельные объекты и составные одинаково.
...
```mermaid
---
title: Composite — диаграмма классов
---
classDiagram
...
**Цель**
Динамически добавлять обязанности объекту.
...
```mermaid
---
title: Decorator — диаграмма классов
---
classDiagram
class InputStream {
<<interface>>
...
```mermaid
---
title: Facade — диаграмма классов
---
classDiagram
class HttpFacade {
get(url)
...
**Цель**
Экономить память, разделяя общее состояние между объектами.
...
```mermaid
---
title: Flyweight — диаграмма классов
---
classDiagram
class Character {
charCode
...
**Как работает**
Прокси реализует тот же интерфейс, что и реальный объект, и может добавлять логику до или после вызова.
...
| 14 | Command                 | Инкапсулирует запрос как объект, позволяя параметризовать клиенты с различными запросами.       | Отмена и повтор действий (Undo/Redo), очередь задач.                             |
| 15 | Iterator                | Предоставляет способ последовательного доступа ко всем элементам коллекции, не раскрывая её внутреннее представление. | `for-each` в Java, интерфейс `Iterable`.                                        |
...
**Цель**
Передавать запрос по цепочке обработчиков.
...
```mermaid
---
title: Chain of Responsibility — диаграмма классов
---
classDiagram
class Handler {
next : Handler
...
```mermaid
---
title: Command — диаграмма классов
---
classDiagram
class Command {
<<interface>>
...
**Цель**
Обеспечить последовательный доступ к элементам коллекции.
...
```mermaid
---
title: Iterator — диаграмма классов
---
classDiagram
class Iterable {
<<interface>>
...
**Цель**
Уменьшить связанность между объектами, заставив их общаться через посредника.
...
```mermaid
---
title: Mediator — диаграмма классов
---
classDiagram
class Mediator {
<<interface>>
...
**Цель**
Сохранять и восстанавливать внутреннее состояние объекта.
...
```mermaid
---
title: Memento — диаграмма классов
---
classDiagram
class Editor {
content
...
**Цель**
Оповещать множество объектов об изменениях состояния.
...
```mermaid
---
title: Observer — диаграмма классов
---
classDiagram
class Subject {
...
**Цель**
Изменять поведение объекта при изменении его состояния.
...
```mermaid
---
title: Strategy — диаграмма классов
---
classDiagram
class Sorter {
...
**Цель**
Определить скелет алгоритма, отложив реализацию шагов в подклассы.
...
```mermaid
---
title: Template Method — диаграмма классов
---
classDiagram
class AbstractTask {
<<abstract>>
...
**Цель**
Добавлять новую операцию к иерархии классов без изменения их кода.
...
```mermaid
---
title: Visitor — диаграмма классов
---
classDiagram
class Visitor {
<<interface>>
...
**Цель**
Определить грамматику языка и интерпретатор для неё.
...
```mermaid
---
title: Interpreter — диаграмма классов
---
classDiagram
class Expression {
<<interface>>
...
Книга **«Design [[pattern|pattern]]s: Elements of Reusable Object-Oriented Software»** (1994) #📘, написанная «Бандой четырёх» (GoF — Gamma #👨, Helm #👨, Johnson #👨, Vlissides #👨), — фундаментальная работа, определившая 23 классических паттерна проектирования.
