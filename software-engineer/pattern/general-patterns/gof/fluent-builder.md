...
## Fluent Builder vs Classic Builder (GoF)

Хотя оба паттерна решают проблему создания сложных объектов, они возникли в разные эпохи и преследуют **разные цели**.

> **Краткая суть:**
> - **Classic Builder ([[gof|GoF]])** — это **структурный паттерн**. Его цель — отделить процесс сборки от представления, чтобы *один и тот же процесс мог создать разные объекты* (например, собрать и деревянный, и каменный дом).
> - **Fluent Builder** — это **паттерн создания API (идиома)**. Его цель — сделать процесс настройки *одного и того же объекта* максимально читаемым и удобным для разработчика за счёт цепочки вызовов ([[fluent-interface|method chaining]]).

## Сравнительная таблица

| Критерий            | Classic Builder (GoF)                                              | Fluent Builder                                                          |
| :------------------ | :----------------------------------------------------------------- | :------------------------------------------------------------------------- |
...
| **Происхождение**   | Книга «Банда четырех» (GoF), 1994 год.                                 | Современная Java-идиома (эволюция GoF + [[fluent-interface|fluent-interface]]).             |
...
## Classic Builder Pattern (GoF)

В классическом понимании (по [[gof|GoF]]) паттерн Builder нужен, когда есть **сложный продукт** и требуется уметь собирать его **разные вариации**, используя пошаговый алгоритм.
...
**Когда применять:** когда нужно собирать **разные по структуре или типу** объекты, используя общий пошаговый алгоритм. В современном Java-бизнес-коде встречается **редко** (чаще в парсерах, генераторах документов, фреймворках).

## Fluent Builder Pattern

Это адаптация, в которой выбрасываются тяжеловесные `Director` и `Abstract Builder`, потому что в 90% задач строить «разные представления» не нужно. Остаётся просто удобно заполнить поля одного DTO или доменного объекта.

Главное правило: **каждый метод-сеттер возвращает сам Builder (`return this`)**, что позволяет писать код в одну строку ([[fluent-interface|Fluent Interface]]).
...
**Когда применять:** практически всегда в бизнес-логике, когда у класса много опциональных полей и нужно сделать код читаемым, а итоговый объект — иммутабельным.
...
| «Мне нужно создать `Query` (SQL, NoSQL, GraphQL), и процесс их сборки сильно отличается, но интерфейс вызова должен быть общим.» | Classic Builder (GoF) |
| «Я хочу, чтобы мой код читался как связный текст: `user.withName().withAge().build()`.» | Fluent Builder |
| «Я пишу парсер HTML, который может собрать из тегов либо DOM-дерево, либо упрощенную текстовую модель.» | Classic Builder (GoF) |

## Эволюция: почему Fluent победил в Java

В классическом [[gof|GoF]] Builder методы часто возвращали `void`:
...
**Итог:** то, что сегодня в Java называется «Builder» (например, `@Builder` от [[lombok|Lombok]] или `Stream.Builder`) — это **Fluent Builder**. Классический GoF Builder остался нишевым инструментом для специфических архитектурных задач.
