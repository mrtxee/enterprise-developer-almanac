...
**Build-Time Композиция** — это подход в [[microfrontend|микрофронтендах]], при котором отдельные приложения (микрофронты) компилируются (**build**) независимо друг от друга, но затем объединяются в единое целое на этапе **сборки (build)**, а не во время выполнения (run-time) в браузере.

Есть 3 варианта реализации:

1. [[module-federation|module-federation]] Build
...
Такая архитектура обеспечит бесшовную интеграцию двух UI-систем в единый интерфейс для пользователя.
