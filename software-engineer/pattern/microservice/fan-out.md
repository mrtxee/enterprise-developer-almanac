...
> «Один отправитель → много получателей» — как вентилятор (fan), который «раздувает» поток воздуха в разные стороны.

### Происхождение термина

Термин **fan-out** заимствован из **электроники и схемотехники**:

- В цифровой электронике **fan-out** — это **максимальное количество входов**, которые может управлять один выход логического элемента (например, один gate управляет 10 входами).
...
```mermaid
---
title: Архитектура модели Fan-out
---
flowchart LR
...
2. **Asynchronous fan-out.** Отправитель отправляет сообщения в очередь/[[message-queueing|брокер]] и сразу продолжает работу, не дожидаясь ответов.
...
6. **Transactional fan-out.**
   - Используется в базах данных для распространения изменений: одна запись в WAL (Write-Ahead Log) может быть обработана несколькими репликами или подписчиками ([[change-data-capture|Change Data Capture]]).
...
2. **Point-to-Multipoint (P2MP)**
   - Один издатель → N подписчиков (типично для **[[publish-subscribe|pub/sub]]**)
3. **Broadcast Fan-out**
...
```mermaid
---
title: Fan-out на практике (уведомления в соцсети)
---
flowchart LR
    A["Пользователь A публикует пост"] --> B[("Message Broker: Kafka topic 'posts'")]
    B --> C["Consumer 1: Feed Service (добавляет в ленту друзей)"]
    B --> D["Consumer 2: Notification Service (отправляет push)"]
    B --> E["Consumer 3: Analytics Service (считает метрики)"]
    B --> F["Consumer 4: Search Indexer (обновляет ElasticSearch)"]
```
...
| **[[microservice|Микросервисная архитектура]]** | Event Sourcing: доменные события → обновление проекций |
...
> - реализовать [[event-driven-architecture|Event-Driven Architecture]] без жёсткой связанности.
