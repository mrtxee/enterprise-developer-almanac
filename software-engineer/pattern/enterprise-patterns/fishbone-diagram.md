...
```mermaid
---
title: Структура диаграммы Исикавы
---
graph LR
    P["Проблема (эффект)"] --> A["Причины: Люди"]
    P --> B["Причины: Процессы"]
    P --> C["Причины: Инструменты"]
    P --> D["Причины: Окружение"]
    P --> E["Причины: Код"]
    P --> F["Причины: Требования"]
```
...
## Как использовать в CI/CD и разработке ПО
...
```mermaid
---
title: Причины падения тестов в CI
---
graph LR
...
```mermaid
---
title: Причины долгого Code Review
---
graph TB
...
Формулируйте проблему конкретно:
...
```mermaid
---
title: Шаблон fishbone-диаграммы
---
graph LR
...
| Ситуация | Пример |
|----------|--------|
| **Постоянно падают тесты** | Flaky tests, проблемы окружения |
