...
**Infrastructure as Code (IaC)** — одна из фундаментальных практик современной DevOps и [[cloud-native|cloud-native]]-разработки.

## Что такое Infrastructure as Code (IaC)

### Определение

> **Infrastructure as Code (IaC)** — практика управления и provisioning IT-инфраструктуры (серверы, сети, балансировщики, базы данных, облачные ресурсы) с помощью **машинно-читаемых файлов кода**, а не ручного взаимодействия или графических интерфейсов.

> То есть: вместо кликов в AWS Console — код на YAML/JSON/HCL, который автоматически создаёт и настраивает инфраструктуру приложения.
...
| ----------------------------------- | ------------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| **Как создавать сервер?**           | Клик в AWS Console, выбор AMI, настройка Security Group, SSH, установка [[nginx|Nginx]]        | Файл `main.tf`, `terraform apply` — всё создано автоматически  |
...
| **Версионность**                    | ❌ Инфраструктура не хранится в [[git|Git]]                                                   | ✅ Всё хранится в Git — как код приложения                      |
...
> IaC превращает инфраструктуру из «черного ящика» в часть продукта.

---

## Почему IaC важен

### Устраняет древнюю проблему

> «На моём компьютере всё работает» → код инфраструктуры в Git — значит, всё работает у всех.
...
- При пуше в Git автоматически запускается `terraform plan`, затем проверка, `apply` и деплой.
...
| Принцип                        | Объяснение                                                                                                                                                               |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Декларативность**            | Описывается **каким должен быть результат**, а не _как его достичь_. Например: _"должна быть 1 [[EC2|EC2]]-машина с Ubuntu 22.04"_ — Terraform сам решит, как это сделать.   |
...
| ---------------------------- | -------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| **[[terraform\|Terraform]]** | HCL (HashiCorp Configuration Language) | **Мультиоблачный** — поддерживает AWS, Azure, GCP, [[kubernetes|Kubernetes]], Cloudflare и т.д. Самый популярный.   | Большинство компаний (Netflix, Spotify, Airbnb)        |
...
> **Ansible, Chef, Puppet — это не чистый IaC**, а **Configuration Management**. Они **настраивают** уже созданные серверы, а не **создают** их. **Terraform/Pulumi — создают**, а **Ansible — настраивает**. Часто используются вместе.
...
**Результат** — за 10 секунд создаётся полностью рабочая инфраструктура в любом регионе и в любой среде.

---

## Пример: IaC для Kubernetes (YAML)
...
```mermaid
---
title: Этапы применения IaC в CI/CD
---
graph LR
...
> **IaC — часть [[CI-CD|CI/CD]]!** Без него нельзя говорить о настоящем [[dev-ops|dev-ops]].
...
|----------|------------------|
| ✅ Локальный проект на одном ноутбуке | Не нужны облака — достаточно [[Docker|Docker]] |
...
> Но при масштабировании IaC становится не опцией, а обязательством.
...
## Финальный вывод: IaC — основа современного облака

| Без IaC | С IaC |
|--------|-------|
| Инфраструктура — «черный ящик» | Инфраструктура — часть продукта |
...
> IaC — это когда инфраструктура перестаёт быть «черным ящиком»: вместо ручного создания серверов — декларативное описание нужных сервисов.
...
```mermaid
---
title: Декларативный и императивный подходы IaC
---
graph TD
...
| **Примеры**                | Terraform, CloudFormation, ARM | [[ansible|ansible]], Chef, Puppet, Bash |
...
```mermaid
---
title: Как декларативный инструмент применяет манифест
---
sequenceDiagram
...
```mermaid
---
title: Сравнение обработки состояния в двух подходах
---
graph TD
...
#### Декларативный подход используют, когда
...
#### Императивный подход используют, когда
