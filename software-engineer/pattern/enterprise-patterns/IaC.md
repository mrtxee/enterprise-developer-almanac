...
**Infrastructure as Code (IaC)** — одна из фундаментальных практик современной DevOps и cloud-native-разработки.
...
| ----------------------------------- | ------------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| **Как создавать сервер?**           | Клик в AWS Console, выбор AMI, настройка Security Group, SSH, установка Nginx        | Файл `main.tf`, `terraform apply` — всё создано автоматически  |
...
| Принцип                        | Объяснение                                                                                                                                                               |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Декларативность**            | Описывается **каким должен быть результат**, а не _как его достичь_. Например: _"должна быть 1 [[EC2]]-машина с Ubuntu 22.04"_ — Terraform сам решит, как это сделать.   |
...
| ------------------------------------ | -------------------------------------- | ---------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| **[[attachments/activities/cource/инженер облачных сетей/Terraform\|Terraform]]** | HCL (HashiCorp Configuration Language) | **Мультиоблачный** — поддерживает AWS, Azure, GCP, Kubernetes, Cloudflare и т.д. Самый популярный.   | Большинство компаний (Netflix, Spotify, Airbnb)        |
...
> **IaC — часть [[ci-cd|CI/CD]]!** Без него нельзя говорить о настоящем [[dev-ops]].
...
| **Примеры**                | [[attachments/activities/cource/инженер облачных сетей/Terraform]], CloudFormation, ARM | [[ansible]], Chef, Puppet, Bash |
...
**Пример: [[attachments/activities/cource/инженер облачных сетей/Terraform]] (декларативный)**
