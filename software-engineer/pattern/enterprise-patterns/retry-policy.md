...
|-----|----------|--------------------|
| **1. Fixed Delay** | Повтор через **фиксированную задержку** (например, каждые 2 секунды) | Простые случаи, когда есть уверенность, что сервис быстро восстановится |
...
| **3. Exponential Backoff + Jitter** | То же, что выше, но со **случайным отклонением** (`jitter`) — например, вместо 4s → 3.7s или 5.2s | Чтобы избежать «thundering herd» — все клиенты не бьют одновременно |
...
| **6. [[circuit-breaker|circuit-breaker]] + Retry** | После N неудач — **перестать пытаться** на время | Если сервис упал — не нужно его забивать запросами |
...
Случайный jitter: вместо 4s → 3.5–4.8s (рандом).

Преимущества:

- Предотвращает **обвал сервера** под грузом повторных запросов.
- Эффективен при **временных сбоях** (сетевые флуктуации, краткие перезагрузки).
- Используется в **Google Cloud**, **AWS**, **[[kafka|Kafka]]**, **[[rabbit-mq|RabbitMQ]]**, **gRPC**.
...
| Код | Причина |
|-----|---------|
| `400 Bad Request` | Ошибка клиента — передавайте запрос правильно |
...
| ✅ **Используйте circuit breaker** | Если несколько попыток подряд неудачны — **не пытайтесь больше** |
...
Сервер проверяет `request_id` → если он уже был — **не снимает деньги второй раз**.
...
| **Fixed Delay** | ⚠️ Только если есть уверенность, что сервис быстро восстановится |
| **No Retry** | ✅ Для HFT, идемпотентных систем |

**Exponential backoff + jitter — это золотой стандарт.** Он **прост**, **эффективен**, **надёжен** и **используется повсюду**.
