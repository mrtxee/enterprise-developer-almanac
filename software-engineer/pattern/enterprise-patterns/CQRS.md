---
aliases:
  - CQRS
  - Command Query Responsibility Segregation
  - Command
  - Query
  - Разделение ответственности команд и запросов
  - Команды и запросы
---

## CQRS — Command Query Responsibility Segregation

**Command Query Responsibility Segregation (CQRS)** — это паттерн проектирования микросервисов, который разделяет операцию чтения и записи данных в системе на два отдельных интерфейса.

Принцип разделения ответственности за выполнение командных запросов.

Типовой пример реализации паттерна CQRS — разделение операций чтения и записи через разные модели данных и интерфейсы.

- **Команды (Commands)** используются для изменения состояния системы.
- **Запросы (Queries)** используются для получения данных из системы.

**Пример**

Служба СДЭК: есть много запросов на отслеживание посылки, которые могут создать проблемы службам курьеров и управления, которые должны вносить изменения в данные.
