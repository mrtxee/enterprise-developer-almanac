...
Service Bus и [[event-bus|event-bus]] — звучат похоже, но на самом деле **относятся к разным уровням абстракции**.
...
| **Примеры** | [[event-driven-architecture|event-driven-architecture]], [[message-broker|message-broker]] как концепция | Kafka, RabbitMQ, AWS EventBridge |
...
| Инструмент | Тип |
|-----------|------|
| **[[kafka|Kafka]]** | Распределённая платформа потоковой обработки |
| **RabbitMQ** | Сообщения + очереди |
| **AWS EventBridge** | Managed event bus для SaaS и [[microservice|микросервисов]] |
...
| **Пример фразы** | «Мы используем Service Bus для интеграции сервисов» | «Мы используем [[kafka|Kafka]] как Event Bus» |
