...
В объектно-ориентированном программировании ([[oop|ООП]]) есть наследование классов, а в реляционных базах данных (РБД) его нет — там есть только плоские таблицы. Задача Inheritance Mappers — сохранить структуру наследования объектов при сохранении их в базу данных.
...
## Single Table Inheritance (Одна таблица на иерархию)
...
## Class Table Inheritance (Таблица на класс)
...
## Concrete Table Inheritance (Таблица на конкретный класс)
...
1. **Single Table** — выбор по умолчанию для большинства случаев (особенно в ORM вроде [[hibernate|Hibernate]] или Entity Framework), если иерархия не слишком сложная. Это самый производительный вариант для чтения.
2. **Class Table** — если у подклассов очень много уникальных полей и не нужно хранить кучу NULL-ов, либо если база данных строго нормализована.
3. **Concrete Table** — используется редко, обычно когда подклассы настолько разные, что к ним почти никогда не обращаются полиморфно (как к базовому типу).

В современных фреймворках ([[django|Django]], Rails, Hibernate) **Single Table Inheritance** часто является настройкой по умолчанию из-за простоты реализации.
