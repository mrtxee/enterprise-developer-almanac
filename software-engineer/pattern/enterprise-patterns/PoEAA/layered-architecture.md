...
адаптер, задача которого — сопоставлять `Entity ⇔ DTO`
...
Слой взаимодействия с данными. При реализации может быть представлен репозиторием.
...
Объект передачи данных. Настраивается для передачи между слоями или для передачи клиенту. Он позволяет контролировать, какие данные получает клиент, для безопасности и консистентности.
