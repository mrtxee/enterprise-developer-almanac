...
**Service Mesh** — это выделенный слой инфраструктуры для управления сетевыми взаимодействиями между [[microservice|микросервисами]]. Он работает вне приложений, как «сетевой транспорт», который автоматически добавляется к сервисам: перехватывает сетевой трафик с помощью [[sidecar|сайдкар]]-прокси, развёрнутых рядом с каждым микросервисом, и централизованно управляет маршрутизацией запросов, балансировкой нагрузки, шифрованием, аутентификацией, логированием и мониторингом — без изменения кода самих сервисов.
...
- **Управление трафиком:** динамическая маршрутизация, «сине-зелёные» и [[canary-release|«канареечные»]] развёртывания с поэтапным разделением трафика.
...
1. **Сайдкары ([[sidecar|sidecar]]):** рядом с каждым микросервисом разворачивается лёгковесный прокси-контейнер, например, на базе [[envoy|Envoy]] — высокопроизводительного прокси-сервера с открытым исходным кодом.
...
| Часть | Описание |
| ----------------- | --------------------------------------------------------------------------------------------------------- |
| **Data Plane** | Сеть **sidecar-прокси** (например, [[envoy|Envoy]]), которые обрабатывают входящий/исходящий трафик каждого сервиса |
...
| Цель | Объяснение |
| ----------------------------------------------- | ---------------------------------------------------------------------------- |
| **Декуплинг логики приложения и сетевых задач** | Не нужно писать retry, timeout, [[circuit-breaker|circuit breaker]] в коде — всё делает mesh |
...
[[istio|Istio]] — один из инструментов, который позволяет организовать [[service-mesh|Service Mesh]] в среде [[kubernetes|Kubernetes]].
