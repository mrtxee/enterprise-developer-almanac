...
**Swiss Cheese Model** (Модель швейцарского сыра) — модель причинности происшествий, предложенная Джеймсом Ризоном (Джеймс Ризон #👨, 1990). Она объясняет, как аварии происходят в сложных системах.
...
| **Дыры** | Слабые места или ошибки в слое | Усталость разработчика, баг в тесте, лаг мониторинга |
...
## Для чего нужна в разработке и CI/CD

| Цель | Описание |
|------|----------|
| **Blameless Culture** | Не искать виноватого (разработчика), а искать дыры в процессах |
...
**Траектория инцидента:** ошибка в коде → пропущена на ревью → не поймана сканером → не замаскирована в логах → **данные утекли**.
...
## Сравнение: Fishbone и Swiss Cheese

| Критерий | Fishbone (Исикава) | Swiss Cheese (Ризон) |
|----------|--------------------|----------------------|
| **Фокус** | Поиск корневой причины | Анализ слоёв защиты |
...
**Идеально использовать вместе:** Fishbone — найти причину, Swiss Cheese — построить защиту от повторения.
...
|------|-----------------------|
| **Виноват разработчик** | **Виновата система:** нет автоматической проверки |
| **Действие:** наказать | **Действие:** добавить SAST в CI |
| **Риск:** разработчик уволится, ошибка повторится | **Риск:** снижен, даже новичок не пропустит |

## Связанные темы

- Шаблон [[Postmortem|Postmortem]] на основе Swiss Cheese.
- Как внедрить [[blameless-culture|Blameless Culture]] в команду.
- Примеры слоёв защиты для [[microservice|микросервисов]].
