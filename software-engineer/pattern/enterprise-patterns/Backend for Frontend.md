...
Для того, чтобы избежать проблем при крос микрофронтенд api запросов при эволюции API создается промежуточный слой, который обрабатывает запросы конкретного фронтенда, — BFF.

Схема взаимодействия фронтендов и бэкендов через BFF:
...
flowchart TB
  ...
  subgraph s2["Бэкенд"]
    G["API Сервис корзины"]
    H["API Сервис каталога"]
    I["API Сервис профиля пользователя"]
    J["API Сервис авторизации"]
  end
