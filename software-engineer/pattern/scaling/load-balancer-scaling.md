...
## Отказоустойчивость внешнего Load Balancer

Классическая проблема: «Кто балансирует балансировщик?» Если внешний LB (с публичным IP) падает — клиенты теряют доступ, даже если бэкенды здоровы. Решение зависит от инфраструктуры.

### Главный принцип

> **Внешний IP-адрес не должен быть привязан к одному физическому или виртуальному устройству.** Он должен «плавать» между узлами или маршрутизироваться на ближайший живой инстанс.
...
### Облачный сценарий: Managed Cloud LB
...
```mermaid
---
title: Мультизональный управляемый Load Balancer
---
flowchart LR
...
### On-premise / Bare-metal сценарий: VRRP + Keepalived
...
```mermaid
---
title: Кластер Load Balancer с VRRP и виртуальным IP
---
flowchart LR
...
### Глобальная доступность: Anycast + BGP
...
```mermaid
---
title: Глобальная маршрутизация по Anycast + BGP
---
flowchart LR
...
### Простой вариант: DNS Failover

**Как работает**

В [[dns|DNS]]-записи `A` указаны несколько внешних IP. При недоступности одного из них (проверяется монитором) провайдер удаляет его из ответа.
...
**Настройка в Cloudflare (Terraform)**
...
- [ ] Конфигурация синхронизирована ([[git|Git]], Ansible, Consul) — чтобы после failover новый мастер работал корректно.
...
| Инфраструктура | Рекомендуемое решение |
|----------------|-----------------------|
...
| **Глобальный сервис с низкими latency-требованиями** | Anycast + BGP (или Cloudflare / AWS Global Accelerator) |
| **Бюджетный проект / MVP** | DNS Failover с низким TTL (60 сек) + мониторинг |

> **Золотое правило:** «Не делайте то, что уже сделал провайдер. Если есть управляемый LB с SLA 99.99% — используйте его.»

### Финальный совет

Начните с **управляемого облачного LB** — это даст отказоустойчивость «из коробки». Если позже понадобится перейти on-premise — уже будет отлаженная логика health-check'ов, синхронизации конфигов и мониторинга, которую можно перенести на VRRP/Keepalived.
