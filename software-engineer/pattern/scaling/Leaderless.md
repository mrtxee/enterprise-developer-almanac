...
**Leaderless-архитектура** — современный подход к построению распределённых систем, в котором **нет центрального управляющего узла (лидера)**. В отличие от традиционных архитектур с лидером (например, Raft, Paxos), здесь **все узлы равны**.

---

## Что такое Leaderless-архитектура?

**Leaderless-архитектура** — это система, где **никакой узел не является «мастером» или «координатором»**, и каждый может принимать запросы на чтение и запись.
...
| Задержка | Может быть высокой (ожидание лидера) | Ниже — можно писать ближе к себе |
...
Пусть есть 5 узлов (`N = 5`).

Вы устанавливаете параметры:
...
```mermaid
---
title: Запись и чтение через кворум (W=3, R=3)
---
graph LR
...
- Запись считается успешной, если **3 из 5** узлов ответили `OK`.
- Чтение — если **3 из 5** узлов дали одинаковый ответ.

Если есть **конфликт** (разные версии) — клиент решает, какую версию взять (last write wins, merge strategy).
...
| **Read Repair** | При чтении обнаруживает разные версии и исправляет их |
...
DynamoDB — одна из первых систем, реализующих leaderless-архитектуру (описана в Dynamo Paper).
...
- Этот узел перешлёт данные тем, кто должен их хранить.
...
| Гибкость | Можно добавлять и удалять узлы |
...
**Leaderless-архитектура** — это когда вы говорите: *«Никто не командует. Мы договорились сами.»*
...
> *«We are willing to sacrifice strong consistency for high availability.»*

Это философия всех leaderless-систем.

**Leaderless-архитектура** — выбор тех, кто ставит доступность выше согласованности. Она позволяет системам **работать почти всегда**, даже если сеть частично недоступна.
...
**Leaderless** и **Multi-Master** — близкие, но не одинаковые понятия. Они оба означают, что **запись возможна на нескольких узлах**, но **отличаются по архитектуре, согласованности и поведению при конфликтах**.
...
| Управление репликацией | Часто синхронная или полусинхронная | Асинхронная, gossip-based |
...
**Multi-Master может быть leaderless, но не обязан. Leaderless — это частный случай Multi-Master с максимальной децентрализацией.**
...
```mermaid
---
title: Репликация между мастерами и репликами
---
graph LR
...
**Определение:** **Leaderless** — это архитектура, где **нет ни одного лидера**, **нет центрального координатора**, и **все узлы равноправны**.
...
```mermaid
---
title: Пример leaderless-сети узлов
---
graph TD
    Client --> NodeA[Node A]
    Client --> NodeB[Node B]
    ...
    style NodeA,NodeB,NodeC,NodeD,NodeE,NodeF fill:#dfe9f5,stroke:#000
```
...
```mermaid
---
title: Multi-Master: мастера общаются напрямую
---
graph LR
    A[Master A] <--> B[Master B]
    A --> C[Slave A]
...
```mermaid
---
title: Leaderless: узлы общаются через gossip
---
graph LR
    A[Cassandra Node]
    B[Cassandra Node]
...
**Сценарий:** пользователи A и B одновременно меняют профиль.

**Multi-Master (MySQL Cluster):**

- Оба пишут — система сравнивает `timestamp`
- Если конфликт — используется **встроенный механизм разрешения**
...
| Сценарий | Рекомендация |
|----------|--------------|
| Нужно много записывающих узлов | **Multi-Master** |
...
| Критерий | Multi-Master | Leaderless |
|--|------------------|----------------|
| Что общего? | Оба позволяют запись на несколько узлов | Оба позволяют запись на несколько узлов |
...
> *«We are moving away from master-slave replication towards a more peer-to-peer system.»*
> — Amazon Dynamo
...
| Leaderless — более радикальная форма multi-master? | ✅ Да |

**Leaderless = Multi-Master + полная децентрализация + eventual consistency + gossip**

**Leaderless** — когда вы говорите: *«Нам не нужен командир. Мы договоримся сами.»*

А **Multi-Master** — это: *«У нас несколько командиров. Но они всё равно советуются.»*
