...
Репликация (англ. [[replication|replication]]) — это создание и поддержание копий базы данных на нескольких серверах.
...
graph LR
    ...
    C --> C1[Много записей<br/>Много чтения]
...
| Использование | Read-интенсивные workload'ы | Распределённые системы |
...
Master (мастер, первичный узел) ответственен за обработку всех операций записи и управление данными.

**Подчинённый узел (slave)**

Slave (слейв, вторичный узел, или реплика) — таких узлов может быть несколько. Они пассивно реплицируют данные от главного узла и обслуживают запросы на чтение в случае применения паттерна read-replica.
...
В случае «потери» мастера следующим мастером становится один из подчинённых узлов, в зависимости от настройки.

Этот подход помогает применять паттерн [[read-replica|read-replica]] — в случае явного перенаправления трафика на чтение с реплики. Это можно сделать как средствами настройки, так и программно на уровне сервиса.

**Архитектура**

```mermaid
---
title: Архитектура read-replica
---
graph TD
...
```mermaid
---
title: Архитектура multi-master
---
graph TD
...
**Bi-directional Replication (BDR)** — механизм, который позволяет данным реплицироваться между несколькими узлами кластера в реальном времени, обеспечивая их синхронизацию и согласованность. В такой архитектуре несколько узлов могут принимать операции чтения и записи, что повышает доступность и масштабируемость системы.

---

## Сравнение multi-master и master-slave
...
- Команда имеет экспертизу в распределённых системах
