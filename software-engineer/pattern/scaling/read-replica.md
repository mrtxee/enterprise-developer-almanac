...
**Паттерн Read Replica (реплика для чтения)** — ключевой подход к повышению производительности, масштабируемости и отказоустойчивости баз данных. Он используется во всех серьёзных системах: от банков до соцсетей.

## Что такое Read Replica

> **Read Replica** — это копия основной (primary) базы данных, настроенная только для операций чтения (SELECT).

Она получает данные с **master-узла (primary)** через **асинхронную [[replication|репликацию]]** и позволяет:
...
| Плюс | Объяснение |
| --- | --- |
| Масштабирование чтения | Можно добавить 10+ реплик и увеличить throughput |
...
| Резервное копирование без блокировок | Бэкапы делаются с реплики — primary не грузится |
...
| Данные могут быть устаревшими | Например, создан заказ и сразу прочитан — на реплике его ещё нет |
...
**На Replica — восстановление из backup + standby mode**
...
| Sharding + Read Replicas | [[sharding|Шардирование]] + реплики внутри шарда — для сверхмасштабирования |
...
| Нужно делать бэкапы | Да — снимайте с реплики |
| Низкий бюджет | Подумайте о кэшировании (Redis) — дешевле |
| Один пользователь | Нет — избыточно |
...
| Не используйте реплику для критичных чтений | Например, после `CREATE ORDER` читать с primary |
...
| Пользователи ждут | Чтение с ближайшей реплики |
...
> «If your database is slow, you have two options: cache or read replicas. Cache for speed. Read replicas for scale.»
...
Read Replica — это когда база данных перестаёт быть «одним сервером» и становится «системой»: масштабируется, живёт дольше, работает быстрее.

> Если не использовать реплики — база строится на одном столбе.
