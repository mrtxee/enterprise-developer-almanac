...
> **Шардирование** — это способ [[data-scaling|масштабирования]] по горизонтали.
...
Деление пользователей по `user_id`:
...
|---------|-------------------|--------------|
| **Физическое распределение** | Может быть в одной БД | Каждый шард на своём сервере |
...
| **Требует изменения приложения** | Иногда | Почти всегда (нужен шард-ключ) |
...
|----------|--------------|
| Большая таблица, медленные запросы | Разбейте на `PARTITION BY date` |
| Нужно быстро удалять старые данные | `DROP TABLE orders_2023` вместо `DELETE` |
| Нет желания менять архитектуру | Работает внутри одного сервера |
| PostgreSQL, Oracle, SQL Server | Все поддерживают партиционирование |

### Когда использовать шардирование

| Сценарий | Рекомендация |
|----------|--------------|
| Нагрузка больше, чем может выдержать один сервер | Шардируйте по `tenant_id`, `user_id` |
| Глобальная система (регионы) | Шарды по регионам: EU, US, Asia |
| SaaS для тысяч клиентов | Один шард на клиента |
| Нужно более 10K RPS | Шардирование — единственный путь |
| Есть ресурсы на DevOps | Сложность высока |
...
| Транзакции | Простые | Сложные ([[two-phase-commit|2PC]], [[saga|Saga]]) |
...
> «Partitioning is about making your queries faster. Sharding is about making your system survive.»
>
...
Шардирование (горизонтальное партиционирование) — ключевая техника для масштабирования баз данных. Сравним основные методы.

### Классификация методов шардирования

```mermaid
---
title: Классификация методов шардирования
---
graph TD
...
|-------|---------|-------|--------|---------------|
| **Range-based** | Диапазоны значений | Простота запросов по диапазону; локальность данных | Hotspots; неравномерное распределение | Аналитика, временные ряды |
| **Hash-based** | Хэш-функция от ключа | Равномерное распределение; нет hotspots | Сложность range-запросов; сложный решардинг | Высоконагруженные OLTP |
| **List-based** | Списки значений | Гибкость группировки; семантическое разделение | Ручное управление; дисбаланс | Мультитенантность, гео-данные |
| **Geo-sharding** | Географическое положение | Низкая задержка; data sovereignty | Сложная синхронизация; глобальные запросы | Глобальные приложения |
| **Directory-based** | Таблица lookup | Гибкость; простой решардинг | Single point of failure; задержка lookup | Динамические схемы |

**Hotspots** — «горячие точки» при шардировании, возникающие из-за неравномерного распределения данных между машинами.

### Range-Based Sharding (по диапазонам)
...
**Особенности**
- ✅ Простые range-запросы
- ✅ Локальность данных
- ❌ Проблемы с hotspots
- ❌ Сложный решардинг

### Hash-Based Sharding (по хэшу)
...
### List-Based Sharding (по спискам)
...
### Geo-Based Sharding (географическое)
...
- ✅ Data sovereignty (соответствие региональным нормам)
- ❌ Сложная синхронизация
- ❌ Глобальные запросы

### Directory-Based Sharding (справочный)
...
**Hotspot-проблема в Range-based шардировании**
...
**Выбирайте Range-based шардирование, когда:**
