...
**PML (Product Maturity Level)** — это система оценки зрелости продукта, которая позволяет руководству и [[change-management|стейкхолдерам]] объективно понимать, на какой стадии развития находится программное решение. Вместо бинарного статуса «готово/не готово», PML использует многомерную шкалу, оценивающую продукт по разным критическим аспектам.
...
Ниже приведено подробное описание уровней зрелости продукта.
...
- **Ключевые метрики:**
  ...
  - Юридическая готовность (типовые договоры, [[distribution-readiness|EULA]], SLA).
...
- **Ключевые метрики:**
  ...
  - **Индекс КБ (Cybersecurity Index):** Результаты пентестов, наличие сертификатов (SOC2, ISO 27001), шифрование данных (at rest/in transit), управление уязвимостями ([[software-qauality-assessment|CVE]]), соответствие регуляторным требованиям ([[distribution-readiness|GDPR]], 152-ФЗ).
- **Низкий уровень:** Монолит, сложно масштабировать, есть известные уязвимости, нет аудита безопасности.
- **Высокий уровень:** [[microservice|Микросервисная]] или событийная архитектура, автоматическое масштабирование, регулярные аудиты безопасности, нулевое доверие (Zero Trust).
...
При отличном продукте с высокой **Функциональной зрелостью** (4.5/5) и хорошей **Архитектурой** (4.0/5), но **Readiness to Sales** на уровне 1.0/5 (продавцы не умеют его продавать), общий коммерческий успех продукта оказывается под угрозой. Управление PML помогает балансировать инвестиции между разработкой, безопасностью, маркетингом и поддержкой.
