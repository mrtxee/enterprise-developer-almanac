...
- Обязательные участники (например, на Review обязательно присутствие Product Owner и ключевых стейкхолдеров).
...
- «Цель спринтов — доставлять сквозные сценарии (end-to-end), а не просто закрывать задачи: каждый спринт должен приносить ценность пользователю».
...
- Код прошёл CI (Sonar/Checkstyle/SpotBugs), нет критических уязвимостей.
- Unit-тесты покрыты на X% для нового кода.
- Пройдены интеграционные тесты для затронутых микросервисов.
- Документация (OpenAPI, README) обновлена.
...
- Шаблоны: как формулировать Sprint Goal, как заполнять Acceptance Criteria.
